...
class Request_model extends CI_Model {
	...
	function get_request_detail($id){
			...
			$wp_company_id =$user->company_id;
             ...
             $this->db->where('request_no', $id);  
			$this->db->where('request.company_id',$wp_company_id);             
             return $this->db->get($this->table_request);
	...
	function get_project_list(){
            ...
            $query = $this->db->query("SELECT project.id, project.project_name FROM project WHERE project.project_status = 1 AND project.wp_company_id=$wp_company_id ORDER BY project_name");
            $rows = array();
   ...
   function get_developer_company_list($user_id){
    ...
    function get_manager_list(){
            ...
            //$query = $this->db->query("SELECT uid,username FROM users LEFT JOIN users_application ON users.uid=users_application.user_id where users_application.application_id=3 and users_application.application_role_id=2 and users_application.company_id=$wp_company_id ORDER BY username");
			$query = $this->db->query("SELECT uid,username FROM users LEFT JOIN users_application ON users.uid=users_application.user_id where users_application.application_id=3 and ( users_application.application_role_id=1 or users_application.application_role_id=2) and users_application.company_id=$wp_company_id ORDER BY username");
            $rows = array();
            foreach ($query->result() as $row){
    ...
    function get_developer_list(){
            ...
            $query = $this->db->query("SELECT uid,username FROM users LEFT JOIN users_application ON users.uid=users_application.user_id where users_application.application_id=3 and users_application.application_role_id=3 and users_application.company_id=$wp_company_id ORDER BY username");
            $rows = array();
            foreach ($query->result() as $row){
                $rows[$row->uid] = $row->username; 
            } 
    ...
    function get_request_no($wp_company_id){
           $sql = "SELECT MAX(request_no) max_request_no FROM request where company_id='".$wp_company_id."'";             
    ...
    public function request_list_search_count_all($user_id, $role_id, $get=NULL,$overdue) {
			$user=  $this->session->userdata('user');  
			$wp_company_id = $user->company_id;
            ...
            if ($get){
				...
				if (isset($get) && !empty($get['start_date']) ){
                $start_date = date("Y-m-d",strtotime(trim($get['start_date'])));
            }
			if (isset($get) && !empty($get['end_date']) ){
                $end_date = date("Y-m-d",strtotime(trim($get['end_date'])));
            }
                if($role_id==3){
                    $assign_developer_id=$user_id;
                }else{
                ...
                if (!empty($request_no)) $cond[] = 'request.request_no = '. intval($request_no).' ';
				...
				//$start_date = date("Y-m-d",strtotime(trim($get['start_date'])));
				//$end_date = date("Y-m-d",strtotime(trim($get['end_date'])));
				 if (!empty($start_date) && !empty($end_date)) { $cond[] = "request_date BETWEEN '$start_date' AND '$end_date' "; }
                 if (!empty($request_priority)) $cond[] = 'priority = '. $request_priority.' ';
                 if (!empty($assign_manager_id)) $cond[] = 'FIND_IN_SET ('.$assign_manager_id.', request.assign_manager_id)';
                 if (!empty($assign_developer_id)) $cond[] = 'FIND_IN_SET('.$assign_developer_id.',request.assign_developer_id)';
			...
			$cond[] = 'project.wp_company_id = '. $wp_company_id;
			
			if(isset($overdue) && $overdue['contractor']){
            	$now = date('Y-m-d');
				$cond[] = 'request.assign_developer_id LIKE "%'.$overdue['contractor'].'%" '; 
				$cond[] = 'request.request_status = 1 ';
                $cond[] = 'request.estimated_completion < "'. $now.'" ';
			}

            $sql = "SELECT COUNT(request_title) total_rows FROM request LEFT JOIN project ON request.project_id=project.id";
	...
	public function request_list_search_count($sort_by = 'request_no', $order_by = 'desc',$offset=0,$limit=10,$user_id, $role_id, $get = NULL,$overdue) {
            $user=  $this->session->userdata('user');  
			$wp_company_id = $user->company_id;
            ...
            if (!empty($request_no)) $this->db->where('request.request_no', intval($request_no)); 
            ...
            if (!empty($assign_developer_id)) $this->db->where("FIND_IN_SET($assign_developer_id, request.assign_developer_id)");           
			 if (!empty($start_date) && !empty($end_date)) $this->db->where(" request_date BETWEEN '$start_date' AND '$end_date'");        
            ...
            if(isset($overdue) && $overdue['contractor']){
            	$now = date('Y-m-d');
				$this->db->like('request.assign_developer_id', $overdue['contractor']); 
				$this->db->where('request.request_status', '1');
				$this->db->where('request.estimated_completion < ', $now);
			}else{
	...
	// Update Notes view
	function notes_view_update($request_id){
        $user = $this->session->userdata('user');
		$uid = $user->uid;
		
		$this->db->select('nid,notes_viewed_byuser'); 
		...
		for($i=0;$i<count($result);$i++){
			$res = '';
			$res = $result[$i]->notes_viewed_byuser;
			$id = $result[$i]->nid;
			if($res==''){$update_ids = $uid;}else{ $update_ids=$res.','.$uid;}
			$list_of_ids = explode(',', $res);
	 		if(in_array($uid, $list_of_ids)){
				continue;
			}else{
				$update_data = array(
					'notes_viewed_byuser' => $update_ids	
				);
				$this->db->where('nid',$id);
				$this->db->update($this->table_task_notes, $update_data);
	...
	function request_hour($id){ 
		$this->db->select("task_hours.*, c.username as contractor, u.username as user");     
