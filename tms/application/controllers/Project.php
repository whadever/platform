...
class Project extends CI_Controller {
	...
	function __construct() {
        ...
        public function project_list($sort_by = 'id', $order_by = 'desc', $offset = 0){
            ...
            $data['privacy_type'] = $this->input->post('privacy_type');
	...
	public function project_add($cid=0){
            ...
            if($this->form_validation->run() === FALSE){
				...
				if($this->input->post('project_start_date')){
					$project_start_date = $this->wbs_helper->to_mysql_date($this->input->post('project_start_date'));
				}else{
					$project_start_date = '0000-00-00';
				}
                ...
                $project_id = $this->project_model->project_save($profile);

				$frequency = $this->input->post('frequency');
				if($frequency == ''){$frequency = 0;}
                else{ $frequency = implode(",", $frequency);}
				...
				$this->project_model->notification_save($notification);
	...
	function project_update($pid){
		...
		if ($this->form_validation->run() === FALSE){
			...
			$data['notification'] = $this->project_model->get_project_notification_details($pid)->row();
			...
			$notification = array(
							'client_name' => $this->input->post('client_name'),
							'notification' => $this->input->post('notification'),
							'frequency' => implode(",", $this->input->post('frequency')),
							'client_email' => $this->input->post('clients_email')
			);
			$this->project_model->notification_update($pid,$notification);
   ...
   function project_detail($pid=0){
	...
	if ($pid <=0){
        ...
        $notification = $this->project_model->get_project_notification_details($pid)->row();
		...
		$this->table->add_row("Client's Name", $notification->client_name);
		$this->table->add_row("Client's Email Address", $notification->client_email);
		...
		if($wp_company_id==111){
			$project_start_date = '';
	        if($project->project_start_date != '0000-00-00'){$project_start_date = date("d-m-Y",strtotime($project->project_start_date));}
	        $this->table->add_row('Project Start Date',$project_start_date);
		}
        ...
        $this->table->add_row('Project Status',$project->project_status==1?'Open':'Completed');
       ...
       $open_tasks_tbl .= "<table class='table'>" .
        ...
        $this->table->add_row(
                ...
                $close_request->request_status==1?'Open':'Completed'
                );
        }
	...
	public function update_project_privacy($project_id,$privacy_type){
		$data = array(
	...
	public function update_privacy_user_ids(){
        $project_id = $_GET['project_id'];
        $cid = urldecode($_GET['cid']);
        
        $add = array(
			'privacy_user_ids' => $cid
		);	 
        $this->db->where('id',$project_id);
        $this->db->update('project',$add);
    }
