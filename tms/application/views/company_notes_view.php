...
<script> 
	...
	$("#submitnote").click(function (e) {
        ...
        $.ajax({  
        url:window.mbsBaseUrl + "company_notes/show_notes/"+pid+"/"+userid,  
        dataType: 'html', 
		data: { notes: mynote}, 
        type: 'GET',   
 ...
 $(document).ready(function() { 
	...
	$("#notes-form").ajaxForm({
                target: '#preview',
                data: {
                    var1: $("#company_id").val()   //assuming #inputText is a text field
                },
                success: function() {
                    refresh_files();
                }
...
window.onload=function(){
    var rid = $('#company_id').val();
    $.ajax({  
        url:window.mbsBaseUrl +"company_notes/show_notes_with_image/"+rid,  
        dataType: 'html',  
        type: 'GET',   
        success:function(data){						  
                         if(data){  	                    
...
<?php
	...
	$this->breadcrumbs->push($company_info->company_name, 'company/company_detail/'.$company_id);	
        $this->breadcrumbs->push('Company Notes', 'company_notes/index/'.$company_id);	
...
<div id="note_page" class="content-inner">    
 ...
 <div id="notes_container_bottom" style=""> 
     <div> 
        <?php 
        $action= base_url().'company_notes/upload_note_image';
