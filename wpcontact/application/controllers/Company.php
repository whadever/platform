...
class Company extends CI_Controller
{
    ...
    public function company_list($sort_by = 'id', $order_by = 'desc', $offset = 0)
    {
        ...
        $this->table->set_heading('#','COMPANY NAME','ADDRESS(S)','CONTACT NUMBER(S)','CITY',/*'COUNTRY',*/'EMAIL','CATEGORY','ACTIONS');
	...
	function company_export_to_excel(){
		$user = $this->session->userdata('user');
		$user_id = $user->uid;
        $wp_company_id = $user->company_id;
		/*getting the data*/		
		...
		$active_sheet->setCellValue('C1', 'Contact Number');
		$active_sheet->setCellValue('D1', 'City');
		$active_sheet->setCellValue('E1', 'Email');
		$active_sheet->setCellValue('F1', 'Category');
		...
		foreach($companies as $company){
			...
			$active_sheet->setCellValue('A'.$i, $company->company_name);
			$active_sheet->setCellValue('B'.$i, $company->company_address);
			$active_sheet->setCellValue('C'.$i, $company->contact_number);
			$active_sheet->setCellValue('D'.$i, $company->company_city);
			$active_sheet->setCellValue('E'.$i, $company->company_email);
			$active_sheet->setCellValue('F'.$i, implode(", ",$tag_links));
			$i++;
		}
		$filename = 'company_list.xls'; //save our workbook as this file name
    ...
    public function add_bulk_company()
    {
        $user = $this->session->userdata('user');
        $user_id = $user->uid;
		$wp_company_id = $user->company_id;

        $data['title'] = 'Add Bulk Company';
        $data['action'] = site_url('company/add_bulk_company');

        $this->_set_rules();

        if ($this->form_validation->run() === FALSE) {
        	
            $data['maincontent'] = $this->load->view('company/add_bulk_company', $data, true);
            $this->load->view('includes/header', $data);
            $this->load->view('home', $data);
            $this->load->view('includes/footer', $data);
			...
			for ($row = 2; $row <= $highestRow; $row++){ 
				...
				for($i=0; $i<count($rowData); $i++){
				    ...
				    $data1['company_name']		=	$rowData[$i][0];
					$data1['company_email']		=	$rowData[$i][1];
					$data1['company_address']	=	$rowData[$i][2];
					$data1['contact_number']		=	$rowData[$i][3];
					$data1['company_city']		=	$rowData[$i][4];
					$data1['company_country']	=	$rowData[$i][5];
					$data1['company_website']	=	$rowData[$i][6];
					$data1['company_notes']		=	$rowData[$i][7];
					
					$data1['category_id']	=	$category_id;
					$data1['wp_company_id']	=	$wp_company_id;
					$data1['status']	=	'1';
					
					$this->db->insert('contact_company' , $data1);
				}
			}
