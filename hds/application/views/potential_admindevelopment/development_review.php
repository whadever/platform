...
<div class="development-add-home development-review" style="background: #fff;">
	...
	<div class="development-body">
		<?php 
			$dev_query = $this->db->query("SELECT dev.*, potential_template.template_name FROM potential_development dev LEFT JOIN potential_template ON dev.tid = potential_template.id where dev.id=$development_id"); 
					...
					<tr>
						...
						<?php 
							...
							for($i = 1; $i <= $number_of_stages; $i++){
								$template_id_query = $this->db->query("SELECT * FROM potential_stage_phase where development_id=$development_id and stage_no=$i");
								$row_template_id = $template_id_query->row();
								if($row_template_id){
									$template_id = $row_template_id->template_id;
									$template_name_query = $this->db->query("SELECT sp.*, potential_template.template_name FROM potential_stage_phase sp LEFT JOIN potential_template ON sp.template_id = potential_template.id where template_id=$template_id and development_id=$development_id and stage_no=$i");
	...
	<div class="development-footer">
		<div class="next-back">
			...
			<a class="next" href="<?php echo base_url();?>potential_admindevelopment/development_list">Finish</a>
			<div class="clear"></div>
		</div>
	</div>
		...
		<div class="dev-tem-review">
			...
			?>
				<div class="review">
					<div class="stage-no">
					<?php
						$stage_no_query = $this->db->query("SELECT * FROM potential_stage_phase where development_id=$development_id and stage_no=$j");
					...
					<?php
						$phase_query = $this->db->query("SELECT * FROM potential_stage_phase where development_id=$development_id and stage_no=$j ORDER BY ordering ASC");
...
<div id="DevView" class="modal dev-stage-tem-review hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	...
	<div class="modal-body">
		<div class="dev-tem-review">
			<div class="reviews">
			<?php
				$phase_query = $this->db->query("SELECT * FROM potential_development_phase where development_id=$development_id ORDER BY ordering ASC");
				$phase_result = $phase_query->result();
				foreach($phase_result as $phase_row){
			?>
				<div class="review">
					...
					<?php
						$phase_id = $phase_row->id;
						$task_query = $this->db->query("SELECT * FROM potential_development_task where development_id=$development_id and phase_id=$phase_id ORDER BY ordering ASC");
						$task_result = $task_query->result();
						foreach($task_result as $task_row){
					?>
						<h6><?php echo $task_row->task_name; ?></h6>
