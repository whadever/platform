...
function changeStateTemplateLoadPhase(template_id,stage_no,development_id)
{
	...
	if (r==true) {
		$.ajax({
			...
			url: window.Url + 'potential_admindevelopment/set_satge_template/' + template_id + '/'+ stage_no + '/'+ development_id,
			type: 'GET',
			success: function(data) 
			{
	 			newurl = window.Url + 'potential_admindevelopment/' + url_stage + '/' + dev_id + '/' + stage_no;
...
<?php
	...
	$ci = & get_instance();
	$ci->load->model('potential_admindevelopment_model');
...
<?php 
	...
	$number_of_stages = $ci->potential_developments_model->get_development_number_of_stage($development_id);	                 
...
jQuery(document).ready(function() {
	if (jQuery('#sortable-phase-<?php echo $i; ?>').length){
		$( "#sortable-phase-<?php echo $i; ?>" ).sortable({
			...
			$.ajax({
				url: window.Url + 'potential_admindevelopment/stage_phase_ordering',
				type: 'POST',
				data: order,
				success: function(data) 
				{
...
<li class="stage stage_<?php echo $i; ?> <?php if(($i) == ($url_stage_no)){ echo 'stage-active'; } ?>">
	<div class="stage-header">
		...
		<h6 style="height:20px">
			...
			<div style="float:left; width:415px">
				<?php 
					$defual_query =$this->db->query("SELECT * FROM potential_stage_phase where development_id=$development_id and stage_no=$i");
					...
					$choice_template_option = $ci->potential_admindevelopment_model->stage_choice_template();
	...
	<?php						
		$phase_query = $this->db->query("SELECT * FROM potential_stage_phase where development_id=$development_id and stage_no=$i ORDER BY ordering ASC");
		...
		<li id="listItemPhase_<?php echo $phase_row->id; ?>" class="accordion <?php if(($phase_row->id) == ($url_phase_id)){ echo 'accordion-active'; } ?>">
			...
			<div class="accordion-header">
				...
				<h6>
					...
					<form class="form-horizontal" action="<?php echo base_url(); ?>potential_admindevelopment/stage_phase_update/<?php echo $phase_row->id; ?>" method="POST">
					...
					<form class="form-horizontal" action="<?php echo base_url();?>potential_admindevelopment/stage_phase_delete/<?php if(isset($phase_row->id)){ echo $phase_row->id; } ?>" method="POST">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
...
jQuery(document).ready(function() {
	if (jQuery("#sortable-task-<?php echo $phase_row->id; ?>").length){
		$( "#sortable-task-<?php echo $phase_row->id; ?>" ).sortable({
			...
			$.ajax({
				url: window.Url + 'potential_admindevelopment/stage_task_ordering',
				type: 'POST',
				data: order,
				success: function(data) 
				{
		...
		$( "#sortable-task-<?php echo $phase_row->id; ?>" ).disableSelection();
				...
				<?php
					...
					$task_query = $this->db->query("SELECT * FROM potential_stage_task where development_id=$development_id and phase_id=$phase_id ORDER BY ordering ASC");
				...
				?>
					<li id="listItemTask_<?php echo $task_row->id; ?>">
						...
						<form class="form-horizontal" action="<?php echo base_url(); ?>potential_admindevelopment/stage_task_update/<?php echo $task_row->id; ?>" method="POST">
							<div class="modal-header">
								<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
						...
						<form class="form-horizontal" action="<?php echo base_url();?>potential_admindevelopment/stage_task_delete/<?php if(isset($task_row->id)){ echo $task_row->id; } ?>" method="POST">
							<div class="modal-header">
								<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
				...
				<li id="droppable-task-<?php echo $phase_row->id; ?>">
					...
					<div id="AddTask_<?php echo $phase_row->id; ?>" class="modal hide fade admindev" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
						<form class="form-horizontal" action="<?php echo base_url();?>potential_admindevelopment/stage_task_add" method="POST">
							<div class="modal-header">
								<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
	...
	<form class="form-horizontal" action="<?php echo base_url();?>potential_admindevelopment/stage_phase_add" method="POST">
		<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
...
?>
	<?php if(isset($admindevelopment->id)) : ?>
	<a class="next" href="<?php echo base_url();?>potential_admindevelopment/development_review_update/<?php echo $development_id; ?>">Next</a>
	<?php else: ?>
	<a class="next" href="<?php echo base_url();?>potential_admindevelopment/development_review/<?php echo $development_id; ?>">Next</a>
	<?php endif; ?>
<?php
echo '<div class="clear"></div>'; 
echo '</div>';
