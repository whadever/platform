...
jQuery(document).ready(function() {
	...
	$("#template_list").change(function() {
		...
		if (r==true) {
			$.ajax({
				url: window.Url + 'potential_admindevelopment/set_development_template/' + dev_id + '/'+ selectedTemplateId,
				type: 'GET',
				success: function(data) 
				{
					newurl = window.Url + 'potential_admindevelopment/' + url_dev + '/' + dev_id + '/' + selectedTemplateId;
...
jQuery(document).ready(function() {
	if (jQuery('#sortable-phase').length){
		$( "#sortable-phase" ).sortable({
			...
			$.ajax({
				url: window.Url + 'potential_admindevelopment/development_phase_ordering',
				type: 'POST',
				data: order,
				success: function(data) 
				{
...
<?php
	...
	$action = 'potential_admindevelopment/development_add_template/save_data';
	
	$ci = & get_instance();
	$ci->load->model('potential_admindevelopment_model');
    $choice_template_option = $ci->potential_admindevelopment_model->development_choice_template();
...
<div class="accordions-body">
		<ul id="sortable-phase" class="accordions">
				...
				<?php
					...
					$phase_query = $this->db->query("SELECT * FROM potential_development_phase where template_id=$template_id and development_id=$development_id ORDER BY ordering ASC");
				...
				<li id="listItemPhase_<?php echo $phase_row->id; ?>" class="accordion <?php if(($phase_row->id) == ($url_phase_id)){ echo 'accordion-active'; } ?>">
					...
					<div class="accordion-header">
						...
						<h6>
							...
							<form class="form-horizontal" action="<?php echo base_url(); ?>potential_admindevelopment/development_phase_update/<?php echo $phase_row->id; ?>" method="POST">
							...
							<form class="form-horizontal" action="<?php echo base_url();?>potential_admindevelopment/development_phase_delete/<?php if(isset($phase_row->id)){ echo $phase_row->id; } ?>" method="POST">
								<div class="modal-header">
									<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
...
jQuery(document).ready(function() {
	if (jQuery("#sortable-task-<?php echo $phase_row->id; ?>").length){
		$( "#sortable-task-<?php echo $phase_row->id; ?>" ).sortable({
			...
			$.ajax({
				url: window.Url + 'potential_admindevelopment/development_task_ordering',
				type: 'POST',
				data: order,
				success: function(data) 
				{
		...
		$( "#sortable-task-<?php echo $phase_row->id; ?>" ).disableSelection();
						...
						<?php
							...
							$task_query = $this->db->query("SELECT * FROM potential_development_task where template_id=$template_id and development_id=$development_id and phase_id=$phase_id ORDER BY ordering ASC");
						...
						?>
							...
							<form class="form-horizontal" action="<?php echo base_url(); ?>potential_admindevelopment/development_task_update/<?php echo $task_row->id; ?>" method="POST">
								<div class="modal-header">
									<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
							...
							<form class="form-horizontal" action="<?php echo base_url();?>potential_admindevelopment/development_task_delete/<?php if(isset($task_row->id)){ echo $task_row->id; } ?>" method="POST">
								<div class="modal-header">
									<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
						...
						<li id="droppable-task-<?php echo $phase_row->id; ?>">
							...
							<div id="AddTask_<?php echo $phase_id; ?>" class="modal hide fade admindev" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
								<form class="form-horizontal" action="<?php echo base_url();?>potential_admindevelopment/development_task_add" method="POST">
									<div class="modal-header">
										<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
				...
				<form class="form-horizontal" action="<?php echo base_url();?>potential_admindevelopment/development_phase_add" method="POST">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
...
?>
	<?php if(isset($admindevelopment->id)) : ?>
	<a class="next" id="next" href="<?php echo base_url();?>potential_admindevelopment/development_add_stage_update/<?php echo $development_id; ?>">Next</a>
	<?php else: ?>
	<a class="next" id="next" href="<?php echo base_url();?>potential_admindevelopment/development_add_stage/<?php echo $development_id; ?>">Next</a>
	<?php endif; ?>
<?php
echo '<div class="clear"></div>'; 
echo '</div>';
