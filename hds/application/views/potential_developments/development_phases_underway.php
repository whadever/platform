...
function change_phase_status(development_id,stage_no,phase_id,checked)
{
	...
	$.ajax({
			url: window.Url + 'potential_developments/update_phase_status/' + phase_id + '/' + checked,
			type: 'GET',
			cache: false,
			success: function(data) 
			{
				//location.reload();
				newurl = window.Url + 'potential_developments/phases_underway/' + development_id + '/' + stage_no;
...
function change_development_phase_task_status(development_id,phase_id,task_id,checked)
{
	...
	$.ajax({
			url: window.Url + 'potential_developments/update_development_phase_task_status/' + task_id + '/' + checked,
			type: 'GET',
			cache: false,
			success: function(data) 
			{
				//alert(data);
				newurl = window.Url + 'potential_developments/phases_underway/' + development_id + '/' + phase_id;
				window.location = newurl;	        
			},
...
function change_all_phase_task_status(development_id,phase_id,checked)
{
	...
	$.ajax({
			url: window.Url + 'potential_developments/update_all_phase_task_status/'+ development_id + '/' + phase_id + '/' + checked,
			type: 'GET',
			cache: false,
			success: function(data) 
			{
...
function change_all_stage_phase_status(development_id,stage_no,checked)
{
	...
	$.ajax({
			url: window.Url + 'potential_developments/update_all_stage_phase_status/'+ development_id + '/' + stage_no + '/' + checked,
			type: 'GET',
			cache: false,
			success: function(data) 
			{
...
jQuery(document).ready(function() {
	...
	<ul id="accordions" class="accordions <?php echo $per; ?>sortable-phase">
		...
		<div class="accordion-header">
				...
				<div class="uncol1" style="width:15%;"><?php if($development_phase_info[$p]->planned_start_date > '0000-00-00'){ echo date('d-m-Y', strtotime($development_phase_info[$p]->planned_start_date)); }else{ echo '00-00-0000'; } ?></div>
				...
				<!-- MODAL Phase Delete-->
					...
					<form class="form-horizontal" action="<?php echo base_url();?>potential_developments/development_phase_delete/<?php echo $development_phase_info[$p]->id; ?>" method="POST">
...
$(function() {
      ...
      enableBrowserDefaultScroll: true
				...
				<!-- MODAL Phase Edit -->
					...
					<form class="form-horizontal" action="<?php echo base_url(); ?>potential_developments/development_phase_update/<?php echo $development_phase_info[$p]->id; ?>" method="POST">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button>
						...
						<div class="modal-body">
							...
							<div class="control-group">
								...
								<div class="controls">
									<input <?php echo $disabled; ?> required="" type="text" class="live_datepicker" name="planned_start_date" value="<?php if($development_phase_info[$p]->planned_start_date > '0000-00-00'){ echo $this->wbs_helper->to_report_date($development_phase_info[$p]->planned_start_date); } ?>">
								</div>
							</div>
							<div class="control-group">
...
<form class="form-horizontal" action="<?php echo base_url(); ?>potential_developments/development_task_add" method="POST">
	<div class="modal-header">
		<button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button>
...
jQuery(document).ready(function() {
	if (jQuery("#sortable-task-<?php echo $development_phase_info[$p]->id; ?>").length){
		...
		<ul id="<?php echo $per; ?>sortable-task-<?php echo $development_phase_info[$p]->id; ?>" class="stage_content dropable-task-<?php echo $development_phase_info[$p]->id; ?>">
		<?php 
			...
			$development_phase_task_info = $ci->potential_developments_model->get_development_phase_task_info($development_id,$development_phase_info[$p]->id)->result();
			...
			for($t = 0; $t < count($development_phase_task_info); $t++)
			{
				...
				if( $development_phase_task_info[$t]->actual_completion_date > '0000-00-00')
				{
...
$(function() {
      ...
      enableBrowserDefaultScroll: true
			...
			<li id="listItemTask_<?php echo $development_phase_task_info[$t]->id; ?>">
					...
					<div class="uncol" style="width:16%"><?php if( $development_phase_task_info[$t]->development_task_status == '1' ) { ?><img style="margin:0 4px 0 0px" width="22" height="22" src="<?php echo base_url();?>images/icon/status_complate.png" /><?php } ?><?php if(isset($development_phase_task_info[$t]->task_name)) { echo $development_phase_task_info[$t]->task_name; } ?></div>				
					...
					<!-- MODAL Task Edit -->
					<div id="DevTaskEdit_<?php echo $development_phase_task_info[$t]->id; ?>" class="modal hide fade stage-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
					<form class="form-horizontal" action="<?php echo base_url(); ?>potential_developments/development_task_update/<?php echo $development_phase_task_info[$t]->id; ?>" method="POST">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button>
						...
						<div class="modal-body">
							...
							<div class="control-group" style="width: 85%;float:left;">
								<label class="control-label" for="task_start_date">Planned Start Date </label>
								<div class="controls">
									<input <?php echo $disabled; ?> required="" type="text" class="live_datepicker" placeholder="" name="task_start_date" value="<?php if($development_phase_task_info[$t]->task_start_date > '0000-00-00'){ echo $this->wbs_helper->to_report_date($development_phase_task_info[$t]->task_start_date); } ?>">
								</div>
							</div>
					...
					<form class="form-horizontal" action="<?php echo base_url();?>potential_developments/development_task_delete/<?php echo $development_phase_task_info[$t]->id; ?>" method="POST">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button>
		...
		{ 
			$ci =&get_instance();
			$ci->load->model('potential_developments_model');
			$phase_info = $ci->potential_developments_model->get_phase_info($development_id,$i)->result(); 
			$all_phase_task = $ci->potential_developments_model->get_all_phase_status($development_id,$i)->result(); 
...
<form class="form-horizontal" action="<?php echo base_url(); ?>potential_developments/development_phase_add" method="POST">
	<div class="modal-header">
		<button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button>
...
<script>
	jQuery(document).ready(function(){
		...
		$('.modal form').ajaxForm({
			success:function(data) {
				newurl = window.Url + 'potential_developments/'+data;
				window.location = newurl;	  
			},			
