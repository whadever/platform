					...
					?>
						...
						?>
							...
							<?php
								...
								foreach($sub_task_result as $task_row){
							?>
								<h6><?php echo $task_row->task_name; ?></h6>
							<?php } ?>
							</div>
		...
		<div class="dev-tem-review">
			...
			?>
				<div class="review">
					...
					<?php
						$phase_id = $phase_row->id;
						$task_query = $this->db->query("SELECT * FROM development_task where development_id=$development_id and phase_id=$phase_id and parent_task_id=0 ORDER BY ordering ASC");
						$task_result = $task_query->result();
						foreach($task_result as $task_row){
					?>
						...
						<div class="review">
							...
							<div class="review-content">
								<?php
									$task_id = $task_row->id;
									$sub_task_query = $this->db->query("SELECT * FROM development_task where development_id=$development_id and phase_id=$phase_id and parent_task_id=$task_id ORDER BY ordering ASC");
									$sub_task_result = $sub_task_query->result();
									foreach($sub_task_result as $task_row){
								?>
									<h6><?php echo $task_row->task_name; ?></h6>
