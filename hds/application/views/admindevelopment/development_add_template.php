...
<?php
	...
	$url_parent_task_id = $this->uri->segment(6);
	...
	$choice_template = form_label('Development Template', 'choice_template');
...
<div class="development-header">
	<div class="development-title">
		...
		<div class="title-inner">Home > development Info > <?php echo $page_title; ?></div>
	</div>
...
<div class="task-phase-header">
	...
	<ul class="drag-phase-task">
		<li id="draggable-sub-task">Add Sub Task</li>
...
<div class="accordions-body">
		<ul id="sortable-phase" class="accordions">
				...
				<?php
					...
					$phase_query = $this->db->query("SELECT * FROM development_phase where template_id=$template_id and development_id=$development_id ORDER BY ordering ASC");
				...
				<li id="listItemPhase_<?php echo $phase_row->id; ?>" class="accordion <?php if(($phase_row->id) == ($url_phase_id)){ echo 'accordion-active'; } ?>">
					...
					<div class="accordion-header">
						...
						<h6>
							...
							<!-- MODAL Phase Edit -->
							<div id="EditPhase_<?php echo $phase_row->id; ?>" class="modal hide fade admindev admindev" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
							<form class="form-horizontal" action="<?php echo base_url(); ?>admindevelopment/development_phase_update/<?php echo $phase_row->id; ?>" method="POST">
								...
								<div class="modal-body">
									...
									<div class="control-group">
										...
										<div class="controls">
											<input required type="text" id="phase_name" name="phase_name" value="<?php if(isset($phase_row->phase_name)){ echo $phase_row->phase_name; } ?>">
										</div>
									</div>
									<div class="control-group">
										<label class="control-label" for="planned_start_date">Planned Start Date </label>
										<div class="controls">
											<input required type="text" class="live_datepicker" name="planned_start_date" value="<?php if($phase_row->planned_start_date > '0000-00-00'){ echo $this->wbs_helper->to_report_date($phase_row->planned_start_date); } ?>">
										</div>
									</div>
									<div class="control-group">
										<label class="control-label" for="planned_finished_date">Planned End date </label>
										<div class="controls">
											<input type="text" class="live_datepicker" name="planned_finished_date" value="<?php if($phase_row->planned_finished_date > '0000-00-00'){ echo $this->wbs_helper->to_report_date($phase_row->planned_finished_date); } ?>">
										</div>
									</div>
...
jQuery(document).ready(function() {
	if (jQuery("#sortable-task-<?php echo $phase_row->id; ?>").length){
		...
		$( "#sortable-task-<?php echo $phase_row->id; ?>" ).disableSelection();
						...
						<ul id="sortable-task-<?php echo $phase_row->id; ?>" class="sub-tasks">
						<?php
							$phase_id = $phase_row->id;
							$task_query = $this->db->query("SELECT * FROM development_task where template_id=$template_id and development_id=$development_id and phase_id=$phase_id and parent_task_id=0 ORDER BY ordering ASC");
							$result_task = $task_query->result();
							$task_row_count = count($result_task);
							foreach($result_task as $task_row){
						?>
							<li id="listItemTask_<?php echo $task_row->id; ?>" class="sub-task <?php if(($task_row->id) == ($url_parent_task_id)){ echo 'sub-task-active'; } ?>">
							
							<div class="sub-task-header">
								<div class="sub-task-icon"></div>
								<h6>
								...
								<form class="form-horizontal" action="<?php echo base_url(); ?>admindevelopment/development_task_update/<?php echo $task_row->id; ?>" method="POST">
									...
									<div class="modal-body">
										...
										<div class="control-group">
											...
											<div class="controls">
												<input required type="text" id="task_name" name="task_name" value="<?php if(isset($task_row->task_name)){ echo $task_row->task_name; } ?>">
											</div>
										</div>
										<div class="control-group">
											<label class="control-label" for="task_start_date">Task Start Date </label>
											<div class="controls">
												<input required class="live_datepicker" type="text" name="task_start_date" value="<?php if($task_row->task_start_date > '0000-00-00'){ echo $this->wbs_helper->to_report_date($task_row->task_start_date); }else if($phase_row->planned_start_date > '0000-00-00'){ echo $this->wbs_helper->to_report_date($phase_row->planned_start_date); } ?>">
											</div>
										</div>
										<div class="control-group">
											<label class="control-label" for="inputPassword"></label>
							...
							<div class="sub-task-content" <?php if(($task_row->id) == ($url_parent_task_id)){ echo 'style="display: block;"'; } else { echo 'style="display: none;"'; } ?>>
							
<script>
jQuery(document).ready(function() {
		
	$( "#draggable-sub-task" ).draggable({
		//connectToSortable: "#sortable-phase",
		helper: "clone",
		revert: "invalid"
	});
	
	$( "#listItemTask_<?php echo $task_row->id; ?>" ).droppable({
		
		drop: function( event, ui ) {
			if (ui.draggable.is('#draggable-sub-task')) {
				$("#listItemTask_<?php echo $task_row->id; ?> .sub-task-content").css("display","block");
				$( this )
				.addClass( "sub-task-active" )
				.find( "li#droppable-sub-task-<?php echo $task_row->id; ?>" )
				.html( '<a href="#AddSubTask_<?php echo $task_row->id; ?>" role="button" data-toggle="modal" class="edit">+ Add New Sub Task</a>' );
			}
		}
	});
								...
								<ul id="sortable-sub-task-<?php echo $task_row->id; ?>" class=""> 
									<?php
										$task_id = $task_row->id;
										$sub_task_query = $this->db->query("SELECT * FROM development_task where template_id=$template_id and development_id=$development_id and phase_id=$phase_id and parent_task_id=$task_id ORDER BY ordering ASC");
										$sub_task_result = $sub_task_query->result();
										$sub_task_row_count = count($sub_task_result);
										foreach($sub_task_result as $sub_task_row){
									?>
									<li id="listItemSubTask_<?php echo $sub_task_row->id; ?>">
										<?php echo $sub_task_row->task_name; ?>
										<a href="#EditSubTask_<?php echo $sub_task_row->id; ?>" title="Task Edit" role="button" data-toggle="modal" class="development-task-edit"><img width="16" height="16" src="<?php echo base_url();?>images/icon/edit_pen.png" /></a>
										<a href="#DeleteSubTask_<?php echo $sub_task_row->id; ?>" title="Task Delete" role="button" data-toggle="modal" class="development-task-delete"><img width="16" height="16" src="<?php echo base_url();?>images/icon/btn_horncastle_trash.png" /></a>
										<!-- MODAL Task Edit -->
										<div id="EditSubTask_<?php echo $sub_task_row->id; ?>" class="modal hide fade admindev" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
										<form class="form-horizontal" action="<?php echo base_url(); ?>admindevelopment/development_task_update/<?php echo $sub_task_row->id; ?>/<?php echo $task_id; ?>" method="POST">
											<div class="modal-header">
												<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
												<h3 id="myModalLabel">Edit Sub Task: <?php if(isset($sub_task_row->task_name)){ echo $sub_task_row->task_name; } ?></h3>
											</div>
											<div class="modal-body">
												
														<input type="hidden" id="task_id" name="task_id" value="<?php if(isset($sub_task_row->id)){ echo $sub_task_row->id; } ?>" readonly="">
													
												<div class="control-group">
													<label class="control-label" for="task_name">Sub Task Name </label>
													<div class="controls">
														<input required type="text" id="task_name" name="task_name" value="<?php if(isset($sub_task_row->task_name)){ echo $sub_task_row->task_name; } ?>">
													</div>
												</div>
												<div class="control-group">
													<label class="control-label" for="task_start_date">Sub Task Start Date </label>
													<div class="controls">
														<input required class="live_datepicker" type="text" name="task_start_date" value="<?php if($sub_task_row->task_start_date > '0000-00-00'){ echo $this->wbs_helper->to_report_date($sub_task_row->task_start_date); }else if($phase_row->planned_start_date > '0000-00-00'){ echo $this->wbs_helper->to_report_date($phase_row->planned_start_date); } ?>">
													</div>
												</div>
												<div class="control-group">
													<label class="control-label" for="inputPassword"></label>
										...
										<!-- MODAL Task Delete-->
										<div id="DeleteSubTask_<?php echo $sub_task_row->id; ?>" class="modal hide fade admindev" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
										<form class="form-horizontal" action="<?php echo base_url();?>admindevelopment/development_task_delete/<?php if(isset($sub_task_row->id)){ echo $sub_task_row->id; } ?>/<?php echo $task_id; ?>" method="POST">
											<div class="modal-header">
												<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
												<h3 id="myModalLabel">Delete Sub Task: <?php if(isset($sub_task_row->task_name)){ echo $sub_task_row->task_name; } ?></h3>
											</div>
											<div class="modal-body">
									...
									?>
									<li id="droppable-sub-task-<?php echo $task_row->id; ?>">
							 
									</li>
									<!-- MODAL Sub Task Add-->
									<div id="AddSubTask_<?php echo $task_row->id; ?>" class="modal hide fade admindev" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
									<form class="form-horizontal" action="<?php echo base_url();?>admindevelopment/development_task_add/<?php echo $task_row->id; ?>" method="POST">
										<div class="modal-header">
											<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
											<h3 id="myModalLabel">Adding a Sub Task</h3>
										</div>
										<div class="modal-body">
											...
											<div class="control-group" style="display:none;">
												...
												<div class="controls">
													<input type="text" id="task_length" name="task_length" value="">
												</div>
											</div>
											...
											<div class="control-group">
												<label class="control-label" for="task_start_date">Task Start Date </label>
												<div class="controls">
													<input required type="text" class="live_datepicker" name="task_start_date" value="">
												</div>
											</div>
											...
											<div class="control-group">
												...
												<div class="controls">
													...
													<input type="hidden" id="template_id" placeholder="" name="template_id" value="<?php echo $template_id; ?>">
													<input type="hidden" id="phase_id" placeholder="" name="phase_id" value="<?php echo $phase_id; ?>">
													<input type="hidden" id="task_ordering" placeholder="" name="task_ordering" value="<?php echo $sub_task_row_count; ?>">
													<input type="hidden" id="url" placeholder="" name="url" value="<?php echo $this->uri->segment(2); ?>">
													<div class="save">
														<input type="submit" value="Submit" name="submit" />
													</div>
				...
				<form class="form-horizontal" action="<?php echo base_url();?>admindevelopment/development_phase_add" method="POST">
					...
					<div class="modal-body">
						...
						<div class="control-group">
							<label class="control-label" for="phase_name">Phase Name </label>
							<div class="controls">
								<input type="text" id="phase_name" name="phase_name" required>
							</div>
						</div>
						<div class="control-group" style="display:none;">
							<label class="control-label" for="phase_length">Phase Length </label>
							<div class="controls">
								<input type="text" id="phase_length" name="phase_length">
							</div>
						</div>
						<div class="control-group">
							<label class="control-label" for="planned_start_date">Planned Start Date </label>
							<div class="controls">
								<input type="text" class="live_datepicker" name="planned_start_date" required>
							</div>
						</div>
						<div class="control-group">
							<label class="control-label" for="planned_finished_date">Planned End date </label>
							<div class="controls">
								<input type="text" class="live_datepicker" name="planned_finished_date">
							</div>
						</div>
						<div class="control-group" style="display:none;">
							<label class="control-label" for="phase_person_responsible">Person Responsible </label>
...
$(document).ready(function () {
	...
	$('.accordions').each(function(){
			...
			$('.sub-tasks').each(function(){
				...
				// Set First Accordion As Active
				$(this).find('.sub-task-header').click(function(){
					
					if(!$(this).parent().hasClass('sub-task-active')){
						
						// Close other accordions
						if(!$(this).parent().parent().hasClass('toggles')){
							$(this).parent().parent().find('.sub-task-active').removeClass('sub-task-active').find('.sub-task-content').slideUp(300);
						}
						
						// Open Accordion
						$(this).parent().addClass('sub-task-active');
						$(this).parent().find('.sub-task-content').slideDown(300);
					
					}else{
						
						// Close Accordion
						$(this).parent().removeClass('sub-task-active');
						$(this).parent().find('.sub-task-content').slideUp(300);
