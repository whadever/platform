...
<div class="development-header">
	<div class="development-title">
		...
		<div class="title-inner">Home > development Info > <?php echo $page_title_before; ?> > <?php echo $page_title; ?></div>
	</div>
...
<li class="stage stage_<?php echo $i; ?> <?php if(($i) == ($url_stage_no)){ echo 'stage-active'; } ?>">
	<div class="stage-header">
		...
		<h6 style="height:20px">
			...
			<div style="float:left; width:415px">
				<?php 
					$defual_query =$this->db->query("SELECT * FROM stage_phase where development_id=$development_id and stage_no=$i");
	...
	<?php						
		$phase_query = $this->db->query("SELECT * FROM stage_phase where development_id=$development_id and stage_no=$i ORDER BY ordering ASC");
...
jQuery(document).ready(function() {
	if (jQuery("#sortable-task-<?php echo $phase_row->id; ?>").length){
		...
		$( "#sortable-task-<?php echo $phase_row->id; ?>" ).disableSelection();
				...
				<div class="stage-sub-tasks">
				
				<ul id="sortable-task-<?php echo $phase_row->id; ?>" class="sub-tasks"> 
				 		  
				<?php
					$phase_id = $phase_row->id;
					$task_query = $this->db->query("SELECT * FROM stage_task where development_id=$development_id and phase_id=$phase_id and parent_task_id=0 ORDER BY ordering ASC");
							...
							<form class="form-horizontal" action="<?php echo base_url(); ?>admindevelopment/stage_task_update/<?php echo $task_row->id; ?>" method="POST">
								...
								<div class="modal-body">
									...
									<div class="control-group">
										...
										<div class="controls">
											<input required type="text" class="live_datepicker" name="task_start_date" value="<?php if($task_row->task_start_date > '0000-00-00'){ echo $this->wbs_helper->to_report_date($task_row->task_start_date); } ?>">
										</div>
									</div>
...
jQuery(document).ready(function() {
			...
			if (ui.draggable.is('#draggable-sub-task')) {
				...
				.html( '<a href="#AddSubTask_<?php echo $task_row->id; ?>" role="button" data-toggle="modal" class="edit">+ Add New Sub Task</a>' );
							...
							<ul id="sortable-sub-task-<?php echo $task_row->id; ?>" class=""> 
								<?php
									$task_id = $task_row->id;
									$sub_task_query = $this->db->query("SELECT * FROM stage_task where development_id=$development_id and phase_id=$phase_id and parent_task_id=$task_id ORDER BY ordering ASC");
									$result_sub_task = $sub_task_query->result();
									$sub_task_row_count = count($result_sub_task);
									foreach($result_sub_task as $sub_task_row){
								?>
									<li id="listItemSubTask_<?php echo $sub_task_row->id; ?>">
										<?php echo $sub_task_row->task_name; ?>
										<a href="#DeleteSubTask_<?php echo $sub_task_row->id; ?>" title="Task Delete" role="button" data-toggle="modal" class="template-task-delete"><img width="16" height="16" src="<?php echo base_url();?>images/icon/btn_horncastle_trash.png" /></a>
										<a href="#EditSubTask_<?php echo $sub_task_row->id; ?>" title="Task Edit" role="button" data-toggle="modal" class="template-task-edit"><img width="16" height="16" src="<?php echo base_url();?>images/icon/edit_pen.png" /></a>
										...
										<form class="form-horizontal" action="<?php echo base_url(); ?>admindevelopment/stage_task_update/<?php echo $sub_task_row->id; ?>/<?php echo $task_id; ?>" method="POST">
											<div class="modal-header">
												<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
											...
											<div class="modal-body">
												...
												<div class="control-group">
													...
													<div class="controls">
														<input required type="text" class="live_datepicker" name="task_start_date" value="<?php if($sub_task_row->task_start_date > '0000-00-00'){ echo $this->wbs_helper->to_report_date($sub_task_row->task_start_date); } ?>">
													</div>
												</div>
										...
										<form class="form-horizontal" action="<?php echo base_url();?>admindevelopment/stage_task_delete/<?php if(isset($sub_task_row->id)){ echo $sub_task_row->id; } ?>/<?php echo $task_id; ?>" method="POST">
											<div class="modal-header">
												<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
								...
								<form class="form-horizontal" action="<?php echo base_url();?>admindevelopment/stage_task_add/<?php echo $task_row->id; ?>" method="POST">
									<div class="modal-header">
										<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
									...
									<div class="modal-body">
										...
										<div class="control-group">
											<label class="control-label" for="task_name">Sub Task Name </label>
											<div class="controls">
												<input required type="text" id="task_name" name="task_name" value="">
											</div>
										</div>
										<div class="control-group" style="display:none;">
				...
				<li id="droppable-task-<?php echo $phase_row->id; ?>">
					...
					<div id="AddTask_<?php echo $phase_row->id; ?>" class="modal hide fade admindev" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	...
	<!-- MODAL Phase -->
	<div id="AddPhase_<?php echo $i; ?>" class="modal hide fade admindev" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
