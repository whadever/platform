		...
		<div class="task-phase-inner">
			...
			<ul id="sortable-phase" class="accordions">
				...
				<?php
					...
					$url_parent_task_id = $this->uri->segment(5);
					$phase_query = $this->db->query("SELECT * FROM template_phase where template_id=$template_id ORDER BY ordering ASC");
				...
				<li id="listItemPhase_<?php echo $phase_row->id; ?>" class="accordion <?php if(($phase_row->id) == ($url_phase_id)){ echo 'accordion-active'; } ?>">
					...
					<div class="accordion-header">
						...
						<h6>
							...
							<form class="form-horizontal" action="<?php echo base_url();?>template/template_phase_delete/<?php if(isset($phase_row->id)){ echo $phase_row->id; } ?>" method="POST">
								<div class="modal-header">
									<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
...
jQuery(document).ready(function() {
	if (jQuery("#sortable-task-<?php echo $phase_row->id; ?>").length){
		...
		$( "#sortable-task-<?php echo $phase_row->id; ?>" ).disableSelection();
						...
						<div class="sub-tasks11">
						
						<ul id="sortable-task-<?php echo $phase_row->id; ?>" class="sub-tasks">
						<?php
							$phase_id = $phase_row->id;
							$task_query = $this->db->query("SELECT * FROM template_task where template_id=$template_id and phase_id=$phase_id and parent_task_id=0 ORDER BY ordering ASC");
				...
				.html( '<a href="#AddSubTask_<?php echo $task_row->id; ?>" role="button" data-toggle="modal" class="edit">+ Add New Sub Task</a>' );
						...
						<li id="listItemTask_<?php echo $task_row->id; ?>" class="sub-task <?php if(($task_row->id) == ($url_parent_task_id)){ echo 'sub-task-active'; } ?>">
							...
							<div class="sub-task-header">
								...
								<form class="form-horizontal" action="<?php echo base_url();?>template/template_task_delete/<?php if(isset($task_row->id)){ echo $task_row->id; } ?>" method="POST">
									<div class="modal-header">
										<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
...
<script>
jQuery(document).ready(function() {
	if (jQuery("#sortable-sub-task-<?php echo $task_row->id; ?>").length){
		$( "#sortable-sub-task-<?php echo $task_row->id; ?>" ).sortable({
			update : function () { 
			var order = $('#sortable-sub-task-<?php echo $task_row->id; ?>').sortable('serialize');
			$.ajax({
				url: window.Url + 'template/template_sub_task_ordering',
				type: 'POST',
				data: order,
				success: function(data) 
				{
		...
		$( "#sortable-sub-task-<?php echo $task_row->id; ?>" ).disableSelection();
	}
});
</script>
								<ul id="sortable-sub-task-<?php echo $task_row->id; ?>" class=""> 
									<?php
										$task_id = $task_row->id;
										$sub_task_query = $this->db->query("SELECT * FROM template_task where template_id=$template_id and phase_id=$phase_id and parent_task_id=$task_id ORDER BY ordering ASC");
									...
									<li id="listItemSubTask_<?php echo $sub_task_row->id; ?>">
										...
										<a href="#EditSubTask_<?php echo $sub_task_row->id; ?>" title="Task Edit" role="button" data-toggle="modal" class="template-task-edit"><img width="16" height="16" src="<?php print base_url(); ?>images/icon/edit_pen.png" /></a>
										<a href="#DeleteSubTask_<?php echo $sub_task_row->id; ?>" title="Task Delete" role="button" data-toggle="modal" class="template-task-delete"><img width="16" height="16" src="<?php print base_url(); ?>images/icon/btn_horncastle_trash.png" /></a>
										<!-- MODAL Sub Task Edit -->
										...
										<form class="form-horizontal" action="<?php echo base_url(); ?>template/template_task_update/<?php echo $sub_task_row->id; ?>/<?php echo $task_id; ?>" method="POST">
											<div class="modal-header">
												<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
												<h3 id="myModalLabel">Edit Sub Task Details</h3>
											</div>
											<div class="modal-body">
												
														<input type="hidden" id="task_id" name="task_id" value="<?php echo $sub_task_row->id; ?>" readonly="">
												<div class="control-group">
													<label class="control-label" for="task_name">Task Name </label>
													<div class="controls">
														<input required type="text" id="task_name" placeholder="" name="task_name" value="<?php echo $sub_task_row->task_name; ?>">
													</div>
												</div>
												<div class="control-group" style="display:none;">
													<label class="control-label" for="task_length">Task Length </label>
													<div class="controls">
														<input type="text" id="task_length" placeholder="" name="task_length" value="<?php echo $sub_task_row->task_length; ?>">
													</div>
												</div>
										...
										<form class="form-horizontal" action="<?php echo base_url();?>template/template_task_delete/<?php if(isset($sub_task_row->id)){ echo $sub_task_row->id; } ?>/<?php echo $task_id; ?>" method="POST">
											<div class="modal-header">
												<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
									...
									?>
									<li id="droppable-sub-task-<?php echo $task_id; ?>">
							 
									</li>
									<!---MODAL Task Add--->
									<div id="AddSubTask_<?php echo $task_id; ?>" class="modal hide fade admindev" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
										<form class="form-horizontal" action="<?php echo base_url();?>template/template_task_add/<?php echo $task_id; ?>" method="POST">
											<div class="modal-header">
												<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
												<h3 id="myModalLabel">New Sub Task Details</h3>
											</div>
											<div class="modal-body">
												...
												<div class="control-group">
													...
													<div class="controls">
														<input type="hidden" id="template_id" placeholder="" name="template_id" value="<?php echo $template_id; ?>">
														<input type="hidden" id="phase_id" placeholder="" name="phase_id" value="<?php echo $phase_id; ?>">
														<input type="hidden" id="phase_no" placeholder="" name="phase_no" value="<?php echo $task_row->phase_no; ?>">
														<input type="hidden" id="task_no" placeholder="" name="task_no" value="<?php echo $sub_task_row_count+1; ?>">
						...
						<li id="droppable-task-<?php echo $phase_row->id; ?>">
							...
							<div id="AddTask_<?php echo $phase_id; ?>" class="modal hide fade admindev" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
								<form class="form-horizontal" action="<?php echo base_url();?>template/template_task_add" method="POST">
									<div class="modal-header">
										<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
				...
				<form class="form-horizontal" action="<?php echo base_url();?>template/template_phase_add" method="POST">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
