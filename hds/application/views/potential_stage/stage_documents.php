		...
		<div class="document-left-bottom">
			<div class="document-table">
				...
				<!-- MODAL Phase Delete-->
					...
					<form class="form-horizontal" action="<?php echo base_url();?>stage/stage_document_update/<?php echo $document->id; ?>" method="POST">
						...
						<div class="modal-footer delete-task">
							...
							<input type="hidden" value="<?php echo $stage_id; ?>" name="stage_no" />
						</div>
					</form>
					</div>
				<!-- MODAL Phase Delete-->
...
?> 
	...
	<div id="delete-document-modal">
		...
		<div class="modal-footer">
			<form action="<?php echo base_url();?>potential_stage/stage_document_delete" method="post">
...
.file_input_hidden  {font-size:45px;position:absolute;right:0px;top:0px;cursor:pointer;
    	...
    	$action =  base_url().'potential_stage/save_stage_document/'.$development_id.'/'.$stage_id;
...
<script>
    function loadDocument(document_id){
        ...
        $.ajax({  
                url: "<?php print base_url(); ?>potential_stage/stage_document_detail/" + document_id, 
                dataType: 'html',  
                type: 'GET',  
...
<script type="text/javascript">
	...
	jQuery(document).ready(function()
	{
		...
		$("#document_search").on("change keyup paste click",function()
		{
			...
			$.ajax({
				url: window.wbsBaseUrl + 'potential_stage/stage_document_search/' + development_id + '/' + stage_no + '/' + encodeURIComponent(search_val),
				type: 'POST',
	...
	$(function() 
	{
		...
		<?php 
			...
			$query = $this->db->query("SELECT * FROM potential_stage_documents where development_id=$development_id and stage_no=$stage_id order by filename_custom ASC");
