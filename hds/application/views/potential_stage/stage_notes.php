...
<div id="development-notes">
    ...
    <div id="devlopment-notes-detail" class="notes-box" >
		...
		<?php 
				foreach($stage_notes as $stage_note){
				echo '<p>Subject: '.$stage_note->notes_title.'</p>';
		        echo '<p>';
		        echo date('d-m-Y', strtotime($stage_note->created)); echo '&nbsp; &nbsp;&nbsp; ';
		        echo date("h:i a", strtotime($stage_note->created));
		        echo '<span style="float:right">Author :'.$stage_note->username.'</span>'; 
		         echo '</p>';
		         echo '<hr style="margin-top:0px;"/>';
		        echo '<p>'.$stage_note->notes_body.'</p>';
				}
		?>
		</div>
    </div>
    <div id="devlopment-notes-search" class="notes-box" style="text-align: center">
        ...
        <form method="post" action="<?php echo base_url().'potential_stage/search_stage_notes/'.$note_development_id.'/'.$stage_id ;?>">
        ...
        <a id="emailnotemessage" href="#">  <?php echo img($email_note); ?></a>
    </div>
    <div class="clear"></div>
    
    <div id="note-message-dialog-box" title="">    
       
        <form id="addmessageform" action="<?php echo base_url();?>potential_stage/send_stage_note_message/<?php echo $note_development_id;?>" method="post">
    ...
    <div id="add-note-dialog-box" title="">
        ...
        <form id="addnoteform" action="<?php echo base_url().'potential_stage/save_stage_note/'.$note_development_id.'/'.$stage_id;?>" method="post">
...
<script>
    function loadNotes(note_id){
        ...
        $.ajax({  
                url: "<?php print base_url(); ?>potential_stage/stage_notes_details/"+note_id,  
                dataType: 'html',  
                type: 'GET',  
