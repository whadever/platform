...
?>
  ...
  <script> 
	...
	$("#submitnote").click(function (e) {
        ...
        $.ajax({  
        url:"<?php echo base_url();?>potential_stage/show_notes/"+rid+"/"+sid+"/"+userid,  
        dataType: 'html',  
...
window.onload=function(){
    var rid = $('#request_id').val();
     var sid = $('#stage_id').val();
    $.ajax({  
        url:"<?php echo base_url();?>potential_stage/show_notes_with_image/"+rid+"/"+sid,  
        dataType: 'html',  
        type: 'GET',   
        success:function(data){						  
                         if(data){  	                    
...
function notesDelete(noteid){
    ...
    if (r == true) {
        $.ajax({  
            url:"<?php echo base_url();?>potential_stage/notes_delete/"+pid+"/"+sid+"/"+noteid,  
            dataType: 'html',  
            type: 'GET',   
            success:function(data){						  
                if(data){                  
...
<?php
	...
	$ci->load->model('potential_stage_model'); 
        $user_option = $ci->potential_stage_model->get_user_list();
...
<div id="note_page">
  ...
  <div <?php echo $per; ?> id="note-bottom" style="background:#fff;"> 
      <div>
    <?php 
    $action= base_url().'potential_stage/upload_note_image';
