...
{
	$.ajax({
			url: window.Url + 'potential_stage/update_task_status/' + task_id + '/' + checked,
			type: 'GET',
			cache: false,
			success: function(data) 
			{
				//location.reload();
				newurl = window.Url + 'potential_stage/phases_list/' + dev_id + '/' + stage_id + '/' + phase_id;
...
function change_all_task_status(development_id,phase_id,stage_no,checked)
{
	...
	$.ajax({
			url: window.Url + 'potential_stage/update_all_task_status/'+ development_id + '/' + phase_id + '/' + stage_no + '/' + checked,
			type: 'GET',
			cache: false,
			success: function(data) 
			{
				//location.reload();
				newurl = window.Url + 'potential_stage/phases_list/' + development_id + '/' + stage_no + '/' + phase_id;
				window.location = newurl;	        
			},
...
jQuery(document).ready(function() {
	...
	<ul id="accordions" class="accordions <?php echo $per; ?>sortable-phase">
		...
		{ 
			...
			$task_info = $ci->potential_stage_model->get_development_stage_phase_task_info($development_id,$stage_id,$phase_info[$i]->id)->result();
			$all_phase_task = $ci->potential_stage_model->get_all_task_status($development_id,$stage_id,$phase_info[$i]->id)->result();
			
			$phase_planned_finished_date = $phase_info[$i]->planned_finished_date;
			
			if( $phase_info[$i]->planned_finished_date > '0000-00-00')
			{
				$planned_finished_date = date('d-m-Y', strtotime($phase_info[$i]->planned_finished_date));
			}
			else if($phase_info[$i]->planned_start_date == '0000-00-00')
			{
		...
		?>
			...
			<div class="accordion-header">
				...
				<div class="uncol1" style="width:15%;"><?php if($phase_info[$i]->planned_start_date > '0000-00-00'){ echo date('d-m-Y', strtotime($phase_info[$i]->planned_start_date)); }else{ echo '00-00-0000'; } ?></div>
				...
				<!-- MODAL Phase Delete-->
					...
					<form class="form-horizontal" action="<?php echo base_url();?>potential_stage/stage_phase_delete/<?php echo $phase_info[$i]->id; ?>" method="POST">
...
$(function() {
      ...
      enableBrowserDefaultScroll: true
					...
					<form class="form-horizontal" action="<?php echo base_url(); ?>potential_stage/stage_phase_update/<?php echo $phase_info[$i]->id; ?>" method="POST">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button>
						...
						<div class="modal-body">
							...
							<div class="control-group">
								...
								<div class="controls">
									<input <?php echo $disabled; ?> required="" type="text" class="live_datepicker" name="planned_start_date" value="<?php if($phase_info[$i]->planned_start_date > '0000-00-00'){ echo $this->wbs_helper->to_report_date($phase_info[$i]->planned_start_date); } ?>">
								</div>
							</div>
							<div class="control-group">
...
<form class="form-horizontal" action="<?php echo base_url(); ?>potential_stage/stage_task_add" method="POST">
	<div class="modal-header">
		<button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button>
...
jQuery(document).ready(function() {
	if (jQuery("#sortable-task-<?php echo $phase_info[$i]->id; ?>").length){
		...
		$( "#sortable-task-<?php echo $phase_info[$i]->id; ?>" ).disableSelection();
			...
			<ul id="<?php echo $per; ?>sortable-task-<?php echo $phase_info[$i]->id; ?>" class="stage_content dropable-task-<?php echo $phase_info[$i]->id; ?>">
				<?php	for($j=0; $j< count($task_info); $j++)
					{ 
						...
						if( $task_info[$j]->planned_completion_date > '0000-00-00')
						{
							$planned_completion_date = date('d-m-Y', strtotime($task_info[$j]->planned_completion_date));
						}
						else if( $task_info[$j]->task_start_date == '0000-00-00')
						{
						...
						elseif( $task_info[$j]->task_start_date == '0000-00-00')
						{
							$rem_days = '';  
							$day_sign = '';
...
$(function() {
      ...
      enableBrowserDefaultScroll: true
				...
				<li id="listItemTask_<?php echo $task_info[$j]->id; ?>">
					...
					<div class="uncol" style="width:16%"><?php if( $task_info[$j]->stage_task_status == '1' ) { ?><img style="margin-right:5px;" width="22" height="22" src="<?php echo base_url();?>images/icon/status_complate.png" /><?php } ?><?php if(isset($task_info[$j]->task_name)) echo $task_info[$j]->task_name; ?></div>				
				...
				<div style="clear:both;"></div>
					...
					<form class="form-horizontal" action="<?php echo base_url(); ?>potential_stage/stage_task_update/<?php echo $task_info[$j]->id; ?>" method="POST">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button>
						...
						<div class="modal-body">
							...
							<div class="control-group" style="width: 85%;float:left;">
								<label class="control-label" for="task_start_date">Planned Start Date </label>
								<div class="controls">
									<input <?php echo $disabled; ?> required="" type="text" class="live_datepicker" name="task_start_date" value="<?php if($task_info[$j]->task_start_date > '0000-00-00'){ echo $this->wbs_helper->to_report_date($task_info[$j]->task_start_date); } ?>">
								</div>
							</div>
							...
							<div class="control-group">
								...
								<div class="controls">
									<select name="task_phase_id" class="form-control" id="task_phase_id">
										...
										$results = $this->db->get('potential_stage_phase')->result();
										foreach($results as $result){
										?>
					...
					<form class="form-horizontal" action="<?php echo base_url();?>potential_stage/stage_task_delete/<?php echo $task_info[$j]->id; ?>" method="POST">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button>
...
$(function() {
      ...
      enableBrowserDefaultScroll: true
		...
		<form class="form-horizontal" action="<?php echo base_url(); ?>potential_stage/stage_phase_add" method="POST">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button>
...
<script>
	jQuery(document).ready(function(){
		...
		$('.modal form').ajaxForm({
			success:function(data) {
				newurl = window.Url + 'potential_stage/'+data;
				window.location = newurl;	  
			},			
