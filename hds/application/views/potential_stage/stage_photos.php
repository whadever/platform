...
<section class="slider">
    ...
    <div class="button-box" style="padding:0px; margin:10px 2px 5px 2px;float: left;">                  	
		...
		<a id="photo-notes" class="btn_dev_info"  href="#">
     	...
     	<a id="photo-archive" class="btn_dev_info"  href="<?php echo base_url();?>potential_stage/stage_archive_photos/<?php echo $development_id;?>/<?php echo $stage_id;?>">
...
$(document).ready(function () {
	$("#photo-notes").click(function () {
			...
			if(photo_id == null){
				...
				window.location = window.wbsBaseUrl+"potential_stage/photo_notes/"+photo_id+"/"+<?php echo $development_id;?>+"/"+<?php echo $stage_id;?>;
			}
	});	
	...
	$("#save-stage").click(
		function () {
			...
			window.location = window.wbsBaseUrl+"potential_stage/pdf_stage_photo/"+photo_id1;
		}
    );
    $("#print-stage").click(
		function () {
			photo_id1 = $('li.flex-active-slide input').val();
			window.open(window.wbsBaseUrl+'potential_stage/print_stage_photo/'+photo_id1, '_blank', 'width=800,height=600,scrollbars=yes,status=yes,resizable=yes,screenx=0,screeny=0');
		}
    );
   ...
   $("#email-stage").click(
		function () {
			...
			$.ajax({
            url: "<?php print base_url(); ?>potential_stage/email_outlook_stage/"+photo_id1,  
                dataType: 'html',  
                type: 'POST',  
	...
	$(".feature_photo").click(function () {
					...
					$.ajax({
		            	url: "<?php print base_url(); ?>potential_stage/photo_action_featured/"+photo_id+"/"+checked,  
		                dataType: 'html',  
		                type: 'POST', 		                 
		                success:     
		                function(data){ 							 
		...
		$(".private_photo").click(function () {
					...
					$.ajax({
		            	url: "<?php print base_url(); ?>potential_stage/photo_action_private/"+photo_id+"/"+checked,  
		                dataType: 'html',  
		                type: 'POST', 		                 
		                success:     
		                function(data){ 							 
...
<div id="delete-photo-modal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	...
	<div class="modal-footer">	
		<form action="<?php echo base_url();?>potential_stage/stage_photo_delete" method="post">
	...
	<div id="photo-message-dialog-box" title="">    
        ...
        <form id="addmessageform" action="<?php echo base_url();?>potential_stage/send_stage_photo_message" method="post">
...
<div id="dialog" title="">
        ...
        <div id='imagebox' style="width:250px; height:300px; border: 1px solid;float: left;"> 
            ...
            <form id="uploadform" action="<?php echo base_url().'potential_stage/upload_stage_photo/'.$development_id.'/'.$stage_id;?>" method="post" enctype="multipart/form-data" >
        ...
        <div style="float:left;width:200px; height: 300px;margin-left: 10px;">
                <form id="photoinfoform" action="<?php echo base_url().'potential_stage/save_stage_photo/'.$development_id.'/'.$stage_id;?>" method="post">
