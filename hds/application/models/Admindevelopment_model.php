...
class AdminDevelopment_model extends CI_Model {
	...
	function development_template_update($did,$tid){
		...
		foreach($phase_ids as $phase)
		{
			...
			$this->db->select('template_id, task_name, task_length, phase_no, ordering, task_no' );
			$this->db->where('template_id', $tid);
			...
			foreach($template_task_data as $tdata)
			{
				...
				$array_task[] = array('development_id' => $did, 'template_id' => $tid, 'phase_id' => $phase_id, 'task_name' => $tdata->task_name, 'task_length' => $tdata->task_length, 'ordering' => $tdata->ordering, 'task_no' => $tdata->task_no); 
		...
		foreach($phase_ids as $phase)
		{
			$phase_id = $phase->id;
			$phase_no = $phase->phase_no;
			$this->db->select('id, task_no');
			$this->db->where('template_id', $tid);
			$this->db->where('phase_no', $phase_no);
			$this->db->where('parent_task_id', '0');
			$dev_task_data = $this->db->get($this->table_template_task)->result();
			foreach($dev_task_data as $tdata)
			{
				$tem_task_id = $tdata->id;
				$task_no = $tdata->task_no;
				
				$this->db->where('development_id', $did);
				$this->db->where('template_id', $tid);
				$this->db->where('phase_id', $phase_id);
				$this->db->where('task_no', $task_no);
				$task_id = $this->db->get($this->table_dev_task)->row()->id;
				
				$this->db->select('task_name, task_length, ordering');
				$this->db->where('template_id', $tid);
				$this->db->where('phase_no', $phase_no);
				$this->db->where('parent_task_id', $tem_task_id);
				$template_sub_task_data = $this->db->get($this->table_template_task)->result();
				foreach($template_sub_task_data as $stdata)
				{
					$array_sub_task[] = array('development_id' => $did, 'template_id' => $tid, 'phase_id' => $phase_id, 'task_name' => $stdata->task_name, 'parent_task_id' => $task_id, 'task_length' => $stdata->task_length, 'ordering' => $stdata->ordering); 
				}
		...
		$this->db->insert_batch($this->table_dev_task, $array_sub_task);
	}
    ...
    function stage_template_update($template_id, $stage_no, $development_id)
	{
		...
		foreach($stage_phase_data as $stage_phase)
		{
			...
			foreach($template_task_data as $tdata)
			{
				...
				$array_stage_task[] = array('development_id' => $development_id, 'template_id' => $template_id, 'phase_id' => $stage_phase->id, 'task_name' => $tdata->task_name, 'task_length' => $tdata->task_length, 'stage_no' => $stage_no, 'ordering' => $tdata->ordering, 'task_no' => $tdata->task_no); 
			}

		}
		
		$this->db->insert_batch($this->table_stage_task, $array_stage_task);
		
		
		foreach($stage_phase_data as $stage_phase)
		{
			$phase_id = $stage_phase->id;	
			$phase_no = $stage_phase->phase_no;	
			...
			foreach($template_task_data as $tdata)
			{
				...
				$this->db->where('task_no', $task_no);
				$this->db->where('stage_no', $stage_no);
				$parent_task_id = $this->db->get($this->table_stage_task)->row()->id;
				//print_r($parent_task_id); exit;
				$this->db->select('template_id, task_name, task_length, phase_no, ordering');
				$this->db->where('template_id', $template_id);
				$this->db->where('phase_no', $phase_no);
				$this->db->where('parent_task_id', $tem_task_id);
				$template_sub_task_data = $this->db->get($this->table_template_task)->result();
				foreach($template_sub_task_data as $tdata)
				{
					$array_stage_sub_task[] = array('development_id' => $development_id, 'template_id' => $template_id, 'phase_id' => $phase_id, 'task_name' => $tdata->task_name, 'task_length' => $tdata->task_length, 'stage_no' => $stage_no, 'ordering' => $tdata->ordering, 'parent_task_id' => $parent_task_id); 
				}
			}

		}
		$this->db->insert_batch($this->table_stage_task, $array_stage_sub_task);
	}
