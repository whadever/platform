<?php 
class Potential_stage_model extends CI_Model {
	...
	private $table_stage_photos= 'potential_stage_photos';
	private $table_stage_photo_notes= 'potential_stage_photo_notes';

	private $table_stage_notes= 'potential_stage_notes';
	private $table_stage_documents= 'potential_stage_documents';
	private $table_users= 'users';
	private $table_stage_phase = 'potential_stage_phase';
	private $table_stage_task = 'potential_stage_task';
	private $table_stage = 'potential_stage';
	private $table_file = 'potential_file';
    private $table_stage_milestone = 'potential_stage_milestone';     
	private $table_stage_phase_dependency = 'potential_stage_phase_dependency';
	...
	function get_development_detail($pid){
 	...
 	public function getStageDocuments($pid, $sid) {
		 $this->db->select('potential_stage_documents.*, users.username as username');			
            $this->db->where('development_id',$pid);   
            $this->db->where('stage_no',$sid); 			 
            $this->db->join('users', 'users.uid = potential_stage_documents.uid', 'left'); 
			$this->db->order_by("id", "desc");       
	...
	public function getStagePhoto($photo_id) {
		    ...
		    $this->db->select('potential_stage_photos.*, users.username');
            $this->db->where('id',$photo_id);  
            $this->db->join('users', 'users.uid = potential_stage_photos.uid', 'left'); 
    ...
    public function getStagePhotos($pid, $sid) {
			...
			$this->db->select('potential_stage_photos.*, users.username as username');
			$this->db->join('users', 'users.uid = potential_stage_photos.uid', 'left'); 
    ...
    function get_stage_notes($pid, $sid){
            $this->db->select('potential_stage_notes.*, users.username as username');
            $this->db->from('potential_stage_notes');
            $this->db->where('project_id',$pid);
            $this->db->where('stage_no',$sid);
            $this->db->join('users', 'users.uid = potential_stage_notes.notes_by', 'left');
	...
	function get_stage_note_detail($did,$stage_no){
	 ...
	 function get_document_detail($document_id){
            $this->db->select('potential_stage_documents.*, users.username as username');
            $this->db->from('potential_stage_documents');
	...
	{
		$this->db->select('`potential_stage_phase.id`, `potential_stage_phase_dependency.dependency_phase_id`, `potential_stage_phase_dependency.dependency_phase_name`, `phase_name`, `users.username`, `phase_no`, `phase_length`, `planned_start_date`, `planned_finished_date`,`actual_finished_date`,`phase_person_responsible`,`phase_status`');
		$this->db->join('users', 'users.uid=potential_stage_phase.phase_person_responsible', 'left');
		$this->db->join('potential_stage_phase_dependency', 'potential_stage_phase_dependency.dependency=potential_stage_phase.id', 'left');
		$this->db->where('development_id', $development_id);
		$this->db->where('stage_no', $stage_no);
	...
	{
		$this->db->select('users.username,`potential_stage_task.id`,`potential_stage_task.task_name`,`potential_stage_task.task_length`,`potential_stage_task.start_alert`,`potential_stage_task.task_start_date`, `potential_stage_task.planned_completion_date`,`potential_stage_task.actual_completion_date`,`potential_stage_task.created`,`potential_stage_task.stage_task_status`,`potential_stage_task.task_person_responsible`');
	...
	{
  		$this->db->select('potential_stage_photos.photo_caption, potential_stage_photos.filename, users.email as useremail');
  		$this->db->from('potential_stage_photos');
  		$this->db->where('id', $photo_id); 
  		$this->db->join('users', 'users.uid = potential_stage_photos.uid'); 
  		$query = $this->db->get();
        return $query;  
 	}
	...
	{
  		$this->db->select('potential_stage_notes.notes_title, potential_stage_notes.notes_by, users.email as useremail');
  		$this->db->from('potential_stage_notes');
  		$this->db->where('nid', $note_id); 
  		$this->db->join('users', 'users.uid = potential_stage_notes.notes_by'); 
  		$query = $this->db->get();
        return $query;  
 	}
	...
	function email_outlook_stage($photo_id){
        ...
        /*stage notes function starts from here */
         function getStageInfo($devid, $stage_id){
             $this->db->select('potential_stage.id, potential_stage.stage_name, c.username as created_by');
              $this->db->join('users c', 'potential_stage.created_by = c.uid', 'left'); 
	...
	function getPriviousStageNotesContractor($devid, $stage_id){
        ...
        function insertNote($rid, $sid, $note, $uid, $notify_user_id, $now, $private){      
            ...
            $this->db->insert('potential_stage_notes', $insert_data);           
	...
	function getPriviousStagePhotoNotes($photo_id){
             $this->db->select('potential_stage_photo_notes.*, users.username');  
              $this->db->join('users', 'potential_stage_photo_notes.notes_by = users.uid', 'left'); 
	...
	function insertPhotoNote($rid, $sid, $note, $uid, $notify_user_id, $now){      
            ...
            $this->db->insert('potential_stage_photo_notes', $insert_data);           
