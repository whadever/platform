<?php 
class Potential_admindevelopment_model extends CI_Model {
	...
	private $table_dev_phase = 'potential_development_phase';
	private $table_dev_task = 'potential_development_task';
    ...
    public function development_list() {
    	...
    	$query = $this->db->query("SELECT dev.*, potential_template.template_name FROM potential_development dev LEFT JOIN potential_template ON dev.tid = potential_template.id WHERE dev.wp_company_id='$wp_company_id' ORDER BY dev.`id` DESC");
        return $query;
	} 
	...
	public function development_feature_photo_insert($file){
        $this->db->insert('potential_development_photos',$file);
        return $this->db->insert_id();            
    }

	public function development_photo_update($fid,$file){ 
		$this->db->where('id',$fid);
		$this->db->update('potential_development_photos',$file);           
    }

	function development_feature_photo_delete($photo_id){
		$this->db->where('id',$photo_id);
		$this->db->delete('potential_development_photos');	
	}
    
    public function development_feature_photo_id($fid){
		$this->db->where('id', $fid);
		return $this->db->get('potential_development_photos');
	}
	...
	function development_choice_template(){
        ...
        $query = $this->db->query("SELECT tep.`template_name`, tep.`id` FROM potential_template tep where template_type=1 and wp_company_id='$wp_company_id' ORDER BY tep.`id` DESC");
        $rows = array();
	...
	function stage_choice_template(){
        ...
        $query = $this->db->query("SELECT tep.`template_name`, tep.`id` FROM potential_template tep where template_type=2 and wp_company_id='$wp_company_id' ORDER BY tep.`id` DESC");
        $rows = array();
        foreach ($query->result() as $row){
            $rows[$row->id] = $row->template_name; 
        }
	...
	function development_phase_ordering(){
		...
		foreach ($_POST['listItemPhase'] as $position => $item){
            $sql = "UPDATE potential_development_phase SET ordering=$position WHERE id=$item";
            $res = mysql_query($sql);
        }
        return $res;
	}
	...
	function development_task_ordering(){
		...
		foreach ($_POST['listItemTask'] as $position => $item){
            $sql = "UPDATE potential_development_task SET ordering=$position WHERE id=$item";
            $res = mysql_query($sql);
        }
        return $res;
	}
	...
	function stage_phase_ordering(){
		//$list $_GET['listItem_'.$phase_id];
		foreach ($_POST['listItemPhase'] as $position => $item){
            $sql = "UPDATE potential_stage_phase SET ordering=$position WHERE id=$item";
            $res = mysql_query($sql);
        }
        return $res;
	}
	...
	function stage_task_ordering(){
		//$list $_GET['listItem_'.$phase_id];
		foreach ($_POST['listItemTask'] as $position => $item){
            $sql = "UPDATE potential_stage_task SET ordering=$position WHERE id=$item";
            $res = mysql_query($sql);
        }
        return $res;
	}
