<?php 
class Potential_developments_model extends CI_Model {
	
	private $primary_key = 'id';
	private $table_development = 'potential_development';
	private $table_file = 'potential_file';
	private $table_notes='potential_notes';
	private $table_development_photos= 'potential_development_photos';
	private $table_development_photo_notes= 'potential_development_photo_notes';

	private $table_development_task = 'potential_development_task';
	private $table_users= 'users';
	private $table_development_phase = 'potential_development_phase';
	private $table_stage_phase = 'potential_stage_phase';
	private $table_development_documents = 'potential_development_documents';
    private $table_development_milestone = 'potential_development_milestone';
	private $table_stage_milestone = 'potential_stage_milestone'; 
	private $table_stage_task = 'potential_stage_task'; 
	...
	public function getDevelopmentPhoto($photo_id) {
		    ...
		    $this->db->select('potential_development_photos.*, users.username');
            $this->db->where('id',$photo_id);  
            $this->db->join('users', 'users.uid = potential_development_photos.uid'); 
	...
	public function getDevelopmentPhotos($pid) {
			...
			$sql = "SELECT id, project_id,filename, photo_caption, uid, stage_no, featured, private, created FROM potential_development_photos as dev_p WHERE dev_p.project_id='$pid'
					UNION
					SELECT id, project_id, filename, photo_caption, uid, stage_no, featured, private, created FROM potential_stage_photos as stage_p WHERE stage_p.project_id='$pid'
    ...
    public function getDevelopmentPhotos_old($pid) {
		    ...
		    $this->db->select('potential_development_photos.*, users.username');
			$this->db->join('users', 'users.uid = potential_development_photos.uid', 'left'); 
			if($user_role==3){
	...
	public function getDevelopmentFeaturePhotos($pid) {
		    ...
		    $this->db->select('potential_development.*, users.username, potential_file.filename');
            $this->db->where('id',$pid);  
            $this->db->join('users', 'users.uid = potential_development.created_by', 'left'); 
			$this->db->join('potential_file', 'potential_file.fid = potential_development.fid', 'left');       
            return $this->db->get($this->table_development);
	}
    ...
    function get_project_notes($pid){
            $this->db->select('potential_notes.*, users.username');
            $this->db->from('potential_notes');
            $this->db->where('project_id',$pid);
            $this->db->join('users', 'users.uid = potential_notes.notes_by', 'left');
	...
	public function get_phase_info($development_id,$stage_no)
	{
		...
		$this->db->join('users', 'users.uid = potential_stage_phase.phase_person_responsible', 'left');
		$this->db->where('development_id', $development_id);
	...
	public function get_development_phase_info($development_id)
	{
		...
		$this->db->join('users', 'users.uid = potential_development_phase.phase_person_responsible', 'left');
  		$this->db->where('development_id', $development_id);
	...
	{
  		$this->db->select('potential_notes.notes_title, potential_notes.notes_by, users.email as useremail');
  		$this->db->from('potential_notes');
  		$this->db->where('nid', $note_id); 
  		$this->db->join('users', 'users.uid = potential_notes.notes_by'); 
  		$query = $this->db->get();
        return $query;  
 	}
	...
	{
  		$this->db->select('potential_development_photos.photo_caption, potential_development_photos.filename, users.email as useremail');
  		$this->db->from('potential_development_photos');
  		$this->db->where('id', $photo_id); 
  		$this->db->join('users', 'users.uid = potential_development_photos.uid'); 
  		$query = $this->db->get();
        return $query;  
	...
	public function get_development_phase_task_info($development_id, $phase_id)
	{
		...
		$this->db->join('users', 'users.uid = potential_development_task.task_person_responsible', 'left'); 
		$this->db->where('development_id', $development_id);
		$this->db->where('phase_id', $phase_id);
		$this->db->order_by('ordering', 'ASC');
		return $this->db->get($this->table_development_task);
	...
	public function getDevelopmentDocuments($did) {
		 	$this->db->select('potential_development_documents.*, users.username');
			//$this->db->from('stage_documents');
			$this->db->where('development_id',$did);
            //$this->db->where('file_category','1');    
            $this->db->join('users', 'users.uid = potential_development_documents.uid'); 
			$this->db->order_by("id", "desc");       
	...
	function get_document_detail($document_id){
        $this->db->select('potential_development_documents.*, users.username');
        $this->db->from('potential_development_documents');
	...
	function email_outlook_development($photo_id){
        ...
        function getDevelopmentsInfo($devid){
             $this->db->select('potential_development.id, potential_development.development_name, c.username as created_by');
              $this->db->join('users c', 'potential_development.created_by = c.uid', 'left'); 
	...
	function getPriviousStageNotesInDev($devid){
            ...
            $this->db->select('potential_stage_notes.*, users.username');  
            $this->db->join('users', 'potential_stage_notes.notes_by = users.uid', 'left'); 
            $this->db->where('project_id', $devid);
			if($user_role==3){
				$this->db->where('private', '0');
			}
            return $this->db->get('potential_stage_notes')->result();                
	} 
	...
	function getPriviousDevelopmentsNotes($devid){
			...
			$user_role = $user->application_role_id;
             ...
             $this->db->select('potential_notes.*, users.username');  
              $this->db->join('users', 'potential_notes.notes_by = users.uid', 'left'); 
            $this->db->where('project_id', $devid);
			if($user_role==3){
				$this->db->where('private', '0');
			}
	...
	function getPriviousDevelopmentphotoNotes($photo_id){
             $this->db->select('potential_development_photo_notes.*, users.username');  
              $this->db->join('users', 'potential_development_photo_notes.notes_by = users.uid', 'left'); 
    ...
    public function getNotifiedUserName($notify_user_id){
        ...
        function insertNote($rid, $note, $uid, $notify_user_id, $now, $private){      
            ...
            $this->db->insert('potential_notes', $insert_data);           
...
function insertPhotoNote($rid, $note, $uid, $notify_user_id, $now){      
            ...
            $this->db->insert('potential_development_photo_notes', $insert_data);           
...
function deleteDevelopmentphotoNotes($noteid){
	...
	public function get_developments_featured_photo($did){
		//echo 'Photo of'.$did;
			...
			$this->db->select('potential_development_photos.filename');
            $this->db->where('potential_development.id',$did);  
            
			$this->db->join('potential_development_photos', 'potential_development_photos.id = potential_development.fid', 'left');       
	...
	function allocation_email_notification_user($development_id,$stage_no){
		...
		$this->db->join('users', 'users.uid = potential_stage_task.task_person_responsible', 'left');		
