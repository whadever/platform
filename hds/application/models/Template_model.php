...
class Template_model extends CI_Model {
	...
	public function save_phase_and_task($template_id,$no_phase,$no_task,$no_sub_tasks){
		$no_phases = $no_phase;
		$no_tasks = $no_task;
		$no_sub_tasks = $no_sub_tasks;
		...
		$this->db->select('id, phase_no, phase_id');
		$this->db->where('template_id', $template_id);
		$template_task_data = $this->db->get($this->table_template_task)->result();

		foreach($template_task_data as $task_data)
		{
			
			for($k = 1; $k <= $no_sub_tasks; $k++)
			{				
				$array_sub_task[] = array('template_id' => $template_id, 'phase_id' => $task_data->phase_id, 'parent_task_id' => $task_data->id, 'phase_no' => $task_data->phase_no, 'task_name' => 'Sub Task Name', 'task_no' => $k); 
				
			}
			
		}
		$this->db->insert_batch($this->table_template_task, $array_sub_task);
    ...
    public function template_list() {
    	...
    	$query = $this->db->query("SELECT temp.*, users.username FROM template temp LEFT JOIN users ON temp.created_by = users.uid WHERE wp_company_id='$wp_company_id' ORDER BY temp.`id` DESC");
        return $query;
	} 
	...
	function template_task_delete($task_id){
		...
		$this->db->where('parent_task_id',$task_id);
		$this->db->delete($this->table_template_task);	
	}
	...
	function template_phase_ordering(){
		...
		foreach ($_POST['listItemPhase'] as $position => $item){
            $sql = "UPDATE template_phase SET ordering=$position WHERE id=$item";
	...
	function template_sub_task_ordering(){
		//$list $_GET['listItem_'.$phase_id];
		foreach ($_POST['listItemSubTask'] as $position => $item){
            $sql = "UPDATE template_task SET ordering=$position WHERE id=$item";
            $res = $this->db->query($sql);
        }
        return $res;
