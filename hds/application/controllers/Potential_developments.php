<?php 
class Potential_developments extends CI_Controller {
	...
	function __construct() {
		...
		$this->load->model('potential_developments_model','',TRUE);
	...
	{
		$this->potential_developments_model->development_document_search($development_id, $search);
	}
	...
	public function developments_list_contractor(){
        ...
        $get = $_GET;
        $developments = $this->potential_developments_model->get_developments_list_contractor();
        $data['developments']=  $developments;
        
        $this->load->view('potential_developments/development_list_contractor',$data);
    ...
    public function change_development_status_contractor($status){
        ...
        $developments = $this->potential_developments_model->change_development_status_contractor($status,$development_city,$development_name);
        //echo $developments;
    ...
    public function index(){
            $data['title'] = 'Project';
            $data['maincontent'] = $this->load->view('potential_developments/developments',$data,true);
        ...
        public function developments_list($sort_by = 'cid', $order_by = 'desc', $offset = 0){
            ...
            $get = $_GET;
            $this->limit = 50;
            $developments = $this->potential_developments_model->get_developments_list($sort_by,$order_by,$offset,$this->limit,$get)->result();
            $data['developments']=  $developments;
		...
		public function developments_list_overview(){
            ...
            $data['title'] = 'Developments';              
            $get = $_GET;
            $developments = $this->potential_developments_model->get_developments_list_overview();
            $data['developments']=  $developments;
            
            $this->load->view('potential_developments/development_list_overview',$data);
		...
		public function development_overview_area(){
            ...
            $developments = $this->potential_developments_model->get_development_overview_area()->result();
            $data['developments']=  $developments;
            
            $data['devlopment_content'] = $this->load->view('potential_developments/development_overview_area',$data,true);
		
            $this->load->view('includes/header',$data);
			$this->load->view('potential_developments/development_overview_home',$data);
            $this->load->view('potential_developments/development_overview_footer',$data);
        }
		...
		public function change_development_status($status){
			...
			$development_name = $get['selectedDevelopmentName2'];
			$development_city = $get['selectedLocationId2'];
            $developments = $this->potential_developments_model->change_development_status($status,$development_city,$development_name);
            //echo $developments;
		...
		public function header_change_development_status($status,$development_city){
			...
			foreach($developments as $development){
				$data .= '<li><a href="'.base_url().'potential_developments/development_detail/'.$development->id.'">'.$development->development_name.'</a></li>';
			}
	...
	public function milestone_delete($did,$mid){
		$this->potential_developments_model->milestone_delete($mid);
		// redirect to project list page
		redirect('potential_developments/development_detail/'.$did);
	}

    function development_detail($pid=0){
		
		if ($pid <=0){
             redirect('potential_developments/developments_list');
        }
        
        $data['development_id']=$pid;
        
        $development = $this->potential_developments_model->get_development_detail($pid)->row();
        $data['development_details'] = $development;

        $feature_photo_id = $development->fid;
        //$data['feature_photo']= $this->potential_developments_model->get_feature_photo($feature_photo_id);
		$data['feature_photos']= $this->potential_developments_model->get_feature_photos($pid);
       
		$development_milestone = $this->potential_developments_model->get_development_milestone_detail($development->id)->row();  
		$development_stage_milestone = $this->potential_developments_model->get_development_stage_milestone_detail($development->id)->result();       
		...
		$data['devlopment_sub_sidebar']=$this->load->view('potential_developments/devlopment_sub_sidebar',$data,true);  
        $data['devlopment_content']=$this->load->view('potential_developments/development_detail',$data,true);
		  
		$this->load->view('includes/header', $data);
		$this->load->view('potential_developments/development_sidebar',$data);
		$this->load->view('potential_developments/development_home',$data);
		$this->load->view('includes/footer', $data);
    }
    public function development_photos($pid=0){
		...
		$data['photos'] = $this->potential_developments_model->getDevelopmentPhotos($pid)->result();

        $data['number_of_stages'] = $this->potential_developments_model->get_development_number_of_stage($pid);
        
        $data['devlopment_sub_sidebar']=$this->load->view('potential_developments/devlopment_sub_sidebar',$data,true); 
        $data['devlopment_content']=$this->load->view('potential_developments/development_photos',$data,true);
	...
	public function development_archive_photos($pid=0){
        ...
        $data['development_id']=$pid;
		$data['photos'] = $this->potential_developments_model->getDevelopmentArchivePhotos($pid)->result();

        $data['number_of_stages'] = $this->potential_developments_model->get_development_number_of_stage($pid);
        
        $data['devlopment_sub_sidebar']=$this->load->view('potential_developments/devlopment_sub_sidebar',$data,true); 
        $data['devlopment_content']=$this->load->view('potential_developments/development_archive_photos',$data,true);
    ...
    public function upload_development_photo($pid=0){
		...
		$config['overwrite'] = TRUE;
         ...
         if ($this->upload->do_upload('photoimg')){
            ...
            $photo_insert_id = $this->potential_developments_model->project_photo_insert($document); 
    ...
    public function save_development_photo($pid=0){
        ...
        $this->potential_developments_model->save_project_photo_info($photo_insert_id, $photo_info);       
              
        redirect('potential_developments/development_photos/'.$pid, 'refresh');          
	...
	public function print_developments_photo($photo_id=0)
	{
	    ...
	    $photo = $this->potential_developments_model->getDevelopmentPhotoDetail($photo_id);     
	    ...
	    $data['table'] = $this->table->generate();
	  	...
	  	$this->load->view('potential_developments/development_photo_print',$data);
    ...
    public function development_overview_old($pid=0){
        ...
        $data['number_of_stages'] = $this->potential_developments_model->get_development_number_of_stage($pid);
        $data['development_overview_info'] = $this->potential_developments_model->get_development_phase_info($pid)->result(); 
        $data['stage_overview_info'] = $this->potential_developments_model->get_development_stage_info($pid)->result();       
        
        $data['devlopment_sub_sidebar']=$this->load->view('potential_developments/devlopment_sub_sidebar',$data,true); 
        $data['devlopment_content']=$this->load->view('potential_developments/development_overview',$data,true);
	...
	public function development_overview($pid=0){
        ...
        $data['development_milestone'] = $this->potential_developments_model->get_development_milestone_detail($pid)->result();

        $data['devlopment_sub_sidebar']=$this->load->view('potential_developments/devlopment_sub_sidebar',$data,true); 
        $data['devlopment_content']=$this->load->view('potential_developments/developments_overview',$data,true);
    ...
    public function development_notes($pid=0){
        ...
        $data['development_id']=$pid;
        $data['notes'] = $this->potential_developments_model->get_project_notes($pid)->result();
		$data['developments_notes'] = $this->potential_developments_model->get_developments_notes($pid)->result();
        $data['number_of_stages'] = $this->potential_developments_model->get_development_number_of_stage($pid);
        
        $data['devlopment_sub_sidebar']=$this->load->view('potential_developments/devlopment_sub_sidebar',$data,true); 
        $data['devlopment_content']=$this->load->view('potential_developments/development_notes',$data,true);
    ...
    public function search_development_notes($pid=0){
        ...
        $data['notes'] = $this->potential_developments_model->get_project_search_notes($pid, $search_notes)->result();

		$data['developments_notes'] = $this->potential_developments_model->get_others_project_search_notes($pid, $search_notes)->result();
		$data['number_of_stages'] = $this->potential_developments_model->get_development_number_of_stage($pid);
    ...
    public function development_notes_details($nid){
        ...
        $note_detail = $this->potential_developments_model->get_note_detail($nid)->row();
        //print_r($note_detail);
    ...
    public function save_development_note($pid=0){
        ...
        $this->potential_developments_model->insert_development_note($note_data);        
        
        redirect('potential_developments/development_notes/'.$pid, 'refresh');
    ...
    public function phases_underway($pid=0){
        ...
        $data['development_id']=$pid;
        $data['stages_no'] = $this->potential_developments_model->get_stage_list($pid)->result();
         //$data['number_of_stages'] = $this->potential_developments_model->get_stage_list($pid)->result();
        //$data['phase_info'] = $this->project->model->get_phase_info($pid)->result();

		$data['development_phase_info'] = $this->potential_developments_model->get_development_phase_info($pid)->result(); 
        $data['number_of_stages'] = $this->potential_developments_model->get_development_number_of_stage($pid);

		
        
        $data['devlopment_sub_sidebar']=$this->load->view('potential_developments/devlopment_sub_sidebar',$data,true); 
        $data['devlopment_content']=$this->load->view('potential_developments/development_phases_underway',$data,true);
...
public function print_development($pid=0)
{
    ...
    $feature_photo_id= $development->fid;
    $data['feature_photo']= $this->potential_developments_model->get_defelopment_feature_photo($feature_photo_id)->row();
    ...
    $this->table->add_row('Geo Tech Engineer', $development->geo_tech_engineer);
  	...
  	$this->load->view('potential_developments/development_print',$data);
 ...
 public function email_development($pid=0){
    ...
    //redirect('potential_developments/development_detail/'.$pid);
    ...
    public function developments_infromation($pid){
        ...
        $feature_photo_id= $development->fid;
    	$feature_photo= $this->potential_developments_model->get_defelopment_feature_photo($feature_photo_id)->row();
	...
	public function update_phase_status($phase_id,$status)
	{
		...
		$this->potential_developments_model->update_status($phase_id,$status);
	}

	public function	update_development_phase_task_status($task_id,$status)
	{
			$this->potential_developments_model->update_development_phase_task_status($task_id,$status);
	}
	...
	public function send_development_note_message($pid)
	{
		...
		$dev_detail= $this->potential_developments_model->get_development_detail($pid)->row();

		$note_detail= $this->potential_developments_model->get_note_author($note_id)->row(); 
		...
		redirect('potential_developments/development_notes/'.$pid.'?sent_email='.$email_send);
    ...
    public function send_development_photo_message()
	{
		...
		$photo_detail= $this->potential_developments_model->get_photo_author($photo_id)->row(); 
		...
		//echo $this->email->print_debugger();
		redirect('potential_developments/development_photos/'.$photo_dev_id.'?sent_email='.$email_send);
	...
	public function phase_task_start_date_update($task_id)
	{
		...
		$this->potential_developments_model->development_phase_task_start_date_update($task_id,$task_data); 
		redirect('potential_developments/phases_underway/'.$dev_id);
	...
	public function development_stage_task_start_date_update($task_id)
	{
		...
		$this->potential_developments_model->development_stage_task_start_date_update($task_id,$task_data); 
		redirect('potential_developments/phases_underway/'.$dev_id);
	...
	public function phase_task_actual_date_update($task_id)
	{
		...
		$this->potential_developments_model->development_phase_task_actual_date_update($task_id,$task_data); 
		redirect('potential_developments/phases_underway/'.$dev_id);
	...
	public function development_stage_task_actual_date_update($task_id)
	{
		...
		$this->potential_developments_model->development_stage_task_actual_date_update($task_id,$task_data); 
		redirect('potential_developments/phases_underway/'.$dev_id);
	...
	public function development_documents($did){
		...
		$development = $this->potential_developments_model->get_development_detail($did)->row();
		$data['title'] = $development->development_name;
		$data['number_of_stages'] = $development->number_of_stages;
		$data['development_id']=$did;

        $data['documents'] = $this->potential_developments_model->getDevelopmentDocuments($did)->result();
        $data['developments_documents'] = $this->potential_developments_model->getOthersDevelopmentDocuments($did)->result();      
                
		$data['number_of_stages'] = $this->potential_developments_model->get_development_number_of_stage($did);

        $data['devlopment_sub_sidebar']=$this->load->view('potential_developments/devlopment_sub_sidebar',$data,true);        
		$data['devlopment_content']=$this->load->view('potential_developments/development_documents',$data,true);
		  
		$this->load->view('includes/header', $data);
		$this->load->view('potential_developments/development_sidebar',$data);
		$this->load->view('potential_developments/development_documents_home',$data);
		$this->load->view('includes/footer', $data);
    }
	...
	public function development_documents_bycategory($did,$cid){
        ...
        $data['documents'] = $this->potential_developments_model->get_development_documents_bycategory($did,$cid)->result();
              
        $data['developments_documents'] = $this->potential_developments_model->getOthersDevelopmentDocumentsBycategory($did,$cid)->result();       
	...
	public function development_document_detail($document_id){
        ...
        $document_detail = $this->potential_developments_model->get_document_detail($document_id)->row();
        //print_r($note_detail);
    ...
    public function save_development_document($did){
        ...
        if ($this->upload->do_upload('upload_document')){
            ...
            $this->potential_developments_model->development_document_insert($document);
			...
			if(!empty($notify_user))
			{
				...
				$notify_user_email = $this->potential_developments_model->notify_one_user_info($notify_user);
				$notify_user_to = $notify_user_email->email;
				$notify_user_name = $notify_user_email->username;

				$dev_info = $this->potential_developments_model->getDevelopmentsInfo($did);
		        ...
		        $message .= " To view this conversation, follow this link: ".base_url()."potential_developments/development_documents/".$did;
				$message .= "</body></html>";	
			...
			redirect('potential_developments/'.$url.'/'.$did.'?error=1');
        } 
        redirect('potential_developments/'.$url.'/'.$did.''.$file_category); 
    }    
    ...
    public function add_new_milestone(){
        ...
        if ($this->input->post('submit')){
            ...
            $this->potential_developments_model->add_new_milestone($add_new_milestone);            
	          	                                     
		} 
        redirect('potential_developments/development_detail/'.$development_id); 
    }
	...
	public function update_milestone($development_id){
        ...
        if ($this->input->post('submit')){
            ...
            $this->potential_developments_model->update_milestone($development_id, $update_milestone);            
	...
	{
		$this->potential_developments_model->update_all_phase_tasks($development_id,$phase_id,$status);
	}
    
	public function update_all_stage_phase_status($development_id,$stage_no,$status)
	{
		$this->potential_developments_model->update_all_satge_phase($development_id, $stage_no,$status);
	}

	public function email_outlook_development($photo_id)
	{
		$this->potential_developments_model->email_outlook_development($photo_id);
	}
	...
	public function development_photo_delete(){
		...
		$this->potential_developments_model->development_photo_delete($development_photo_id);
		redirect('potential_developments/development_photos/'.$development_id);
	}
	...
	public function development_archive_photo_delete(){
		...
		$development_photo_id = $post['dev_photo_id'];
		$this->potential_developments_model->development_photo_delete($development_photo_id);
		redirect('potential_developments/development_archive_photos/'.$development_id);
	}
	...
	public function development_document_delete(){
		...
		$this->potential_developments_model->development_document_delete($dev_document_id);
		redirect('potential_developments/development_documents/'.$development_id, 'refresh');
	}
	...
	public function development_update($development_id) {   
        ...
        if ( $this->input->post('submit')) {
			...
			if ($this->upload->do_upload('feature_photo')){
	            ...
	            $photo_insert_id = $this->potential_developments_model->development_feature_photo_insert($file); 
		    ...
		    $id = $this->potential_developments_model->development_update($development_id,$development_update);
			
		} 
		
		redirect('potential_developments/development_detail/'.$development_id, 'refresh');
	}
  public function notes($dev_id='')
  {
      //echo $req_id;
      
      $development = $this->potential_developments_model->get_development_detail($dev_id)->row();
	  $data['title'] = $development->development_name;
 
      $data['development_id']=$dev_id;
      $data['number_of_stages'] = $this->potential_developments_model->get_development_number_of_stage($dev_id);
      $data['request_info']= $this->potential_developments_model->getDevelopmentsInfo($dev_id);
      //print_r($data['request_info']); exit;

		$prev_notes = $this->potential_developments_model->getPriviousDevelopmentsNotes($dev_id);
		$prev_notes_stage = $this->potential_developments_model->getPriviousStageNotesInDev($dev_id);

		//print_r($prev_notes_stage);exit;
		$data['feature_photos']= $this->potential_developments_model->get_feature_photos($dev_id);
      ...
      //print_r($prev_notes);
       ...
       $data['devlopment_sub_sidebar']=$this->load->view('potential_developments/devlopment_sub_sidebar',$data,true); 
       $data['devlopment_content']=$this->load->view('potential_developments/development_notes_view',$data,true);
	...
	public function photo_notes($photo_id='', $dev_id='')
 	{
      ...
      $data['development_id']=$dev_id;
		$data['photo'] = $this->potential_developments_model->getDevelopmentPhoto($photo_id)->row();

      $data['number_of_stages'] = $this->potential_developments_model->get_development_number_of_stage($dev_id);
      $data['request_info']= $this->potential_developments_model->getDevelopmentsInfo($dev_id);
      //print_r($data['request_info']); exit;
      $prev_notes = $this->potential_developments_model->getPriviousDevelopmentphotoNotes($dev_id);
      $data['prev_notes'] = $this->notes_image_tmpl($prev_notes);
      //print_r($prev_notes); exit;
      
       $data['devlopment_sub_sidebar']=$this->load->view('potential_developments/devlopment_sub_sidebar',$data,true); 
       $data['devlopment_content']=$this->load->view('potential_developments/development_photo_notes_view',$data,true);
	...
	public function show_notes_with_image($rid){ 
		...
		$prev_notes= $this->potential_developments_model->getPriviousDevelopmentsNotes($rid);
		$prev_notes_stage = $this->potential_developments_model->getPriviousStageNotesInDev($rid);
  ...
  public function show_photo_notes_with_image($rid){ 
      ...
      $prev_notes= $this->potential_developments_model->getPriviousDevelopmentphotoNotes($rid);
  ...
  public function notes_image_tmpl($prev_notes,$prev_notes_stage){
	  ...
	  foreach ($prev_notes_stage as $notes) {
        ...
        {
           $showuser= 'Me'; 
           $notified_user= $this->potential_developments_model->getNotifiedUserName($notes->notify_user_id);
	...
	public function show_notes($rid, $notify_user_id){
		...
		$note7 = str_replace("back_slash", "\\", $note6);
      	...
      	$request_info= $this->potential_developments_model->getDevelopmentsInfo($rid);      
        ...
        $notify_user_info=$this->potential_developments_model->get_user_info($notify_user_id);        
        ...
        $message2 .= " To view this conversation, follow this link: ".base_url()."potential_developments/notes/".$rid."";
		$message2 .= "</body></html>";	
		//$msg_body='message body';
		$msg_body2=$message2;
        mail($notify_user_to, $subject2, $msg_body2, $headers2);

        $insert_note = $this->potential_developments_model->insertNote($rid, $note0, $user_id, $notify_user_id, $now, $private);      
        $prev_notes= $this->potential_developments_model->getPriviousDevelopmentsNotes($rid);
        echo $this->notes_image_tmpl($prev_notes); 
	...
	public function notes_delete($pid, $noteid){ 
      ...
      $this->potential_developments_model->deleteDevelopmentsNotes($noteid);
      $prev_notes= $this->potential_developments_model->getPriviousDevelopmentsNotes($pid);
      echo $this->notes_image_tmpl($prev_notes);
      
  	}
	public function photo_action_featured($photo_id, $checked_val){
		
		$this->potential_developments_model->update_photo_featured($photo_id, $checked_val);
	}
	public function photo_action_private($photo_id, $checked_val){
		
		$this->potential_developments_model->update_photo_private($photo_id, $checked_val);
	}
	public function insert_photo_notes($rid, $notify_user_id){
		...
		$note = $_GET['notes']; 
      	...
      	$note_body = $note;
		$insert_note = $this->potential_developments_model->insertPhotoNote($rid, $note0, $user_id, $notify_user_id, $now);      
        $prev_notes= $this->potential_developments_model->getPriviousDevelopmentphotoNotes($rid);
        echo $this->notes_image_tmpl($prev_notes); 
        ...
        $message2 .= 'Development : '.$request_title.'<br />';
		$message2 .= "Note Description: " . $note_body . " <br />";
        $message2 .= " To view this conversation, follow this link: ".base_url()."potential_developments/photo_notes/".$rid."";
		$message2 .= "</body></html>";	
		//$msg_body='message body';
		$msg_body2=$message2;
        mail($notify_user_to, $subject2, $msg_body2, $headers2);
	...
	public function photo_notes_delete($pid, $noteid){ 
      ...
      $this->potential_developments_model->deleteDevelopmentphotoNotes($noteid);
      $prev_notes= $this->potential_developments_model->getPriviousDevelopmentphotoNotes($pid);
      echo $this->notes_image_tmpl($prev_notes);
	...
	public function development_phase_delete($phase_id){      
      	$post = $this->input->post();
		$development_id = $post['development_id'];  
		$this->potential_developments_model->development_phase_delete($phase_id);  
		echo 'phases_underway/'.$development_id.'/'.$phase_id;
		//redirect('potential_developments/phases_underway/'.$development_id.'/'.$phase_id, 'refresh'); 
  	}
	...
	public function development_phase_update($phase_id){
        ...
        if ($this->input->post('submit')){
            ...
            $this->potential_developments_model->development_phase_update($phase_id,$update_phase); 
			...
			if(!empty($notify_user))
			{
				...
				$dev_info = $this->potential_developments_model->getDevelopmentsInfo($development_id);
				$dev_name = $dev_info->development_name;
		        ...
		        $message .= "To view this allocation, follow this link: ".base_url()."potential_developments/phases_underway/".$development_id."/".$phase_id;
				$message .= "</body></html>";	
	...
	public function development_phase_add(){
        ...
        if ($this->input->post('submit')){
            ...
            $id = $this->potential_developments_model->development_phase_add($add_phase); 
			...
			if(!empty($notify_user))
			{
		        ...
		        $message .= 'You have been allocated '.$phase.' in '.$dev_name.'.<br />';
				$message .= "Please ensure you are monitoring your task(s) correctly.<br />";
		        $message .= "To view this allocation, follow this link: ".base_url()."potential_developments/phases_underway/".$development_id."/".$id;
				$message .= "</body></html>";	
	        ...
	        //redirect('potential_developments/phases_underway/'.$development_id.'/'.$id, 'refresh');  	                                     
		} 
	...
	public function development_task_delete($task_id){      
		...
		$this->potential_developments_model->development_task_delete($task_id);  
		echo 'phases_underway/'.$development_id.'/'.$phase_id;
		//redirect('potential_developments/phases_underway/'.$development_id.'/'.$phase_id, 'refresh'); 
  	}
	...
	public function development_task_update($task_id){
        ...
        if ($this->input->post('submit')){
            ...
            $this->potential_developments_model->development_task_update($task_id,$update_task); 
			...
			if(!empty($notify_user))
			{
				...
				$phase_info = $this->potential_developments_model->getDevelopmentPhaseInfo($phase_id);
	...
	public function development_task_add(){
        ...
        if ($this->input->post('submit')){
            ...
            $this->potential_developments_model->development_task_add($add_task);   
	...
	public function show_developments_featured_photo($did){
		...
		$photo = $this->potential_developments_model->get_developments_featured_photo($did); 
	...
	public function development_document_update($id){
        ...
        if ($this->input->post('submit')){
            ...
            $this->potential_developments_model->development_document_update($id,$update);            
	...
	function allocation_email_notification($development_id,$stage_no,$person_responsible){
		...
		$users = $this->potential_developments_model->allocation_email_notification_user($development_id,$stage_no)->result();	
