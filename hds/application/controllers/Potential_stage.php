<?php 
class Potential_stage extends CI_Controller {
	...
	function __construct() {
		...
		$this->load->model('potential_stage_model','',TRUE);
    ...
    public function index(){
            $data['title'] = 'Stage';
            $data['maincontent'] = $this->load->view('potential_stage/stages',$data,true);
	...
	{
		$this->potential_stage_model->stage_document_search($development_id, $stage_id, $search);
	}
   ...
   public function stage_add() {        
        ...
        if ( $this->input->post('submit')) {
			...
			if ($this->upload->do_upload('feature_photo')){
	            ...
	            $photo_insert_id = $this->potential_stage_model->stage_feature_photo_insert($document); 	             
		    ...
		    $this->potential_stage_model->stage_add($stage_add);

			redirect('potential_stage/stage_info/'.$development_id.'/'.$stage_no);
	...
	public function stage_update($stage_id) {        
        ...
        if ( $this->input->post('submit')) {
			...
			if ($this->upload->do_upload('feature_photo')){
				...
				$this->potential_stage_model->stage_feature_photo_delete($photo_insert_id); 
	            $photo_insert_id = $this->potential_stage_model->stage_feature_photo_insert($document); 	             
		    ...
		    $this->potential_stage_model->stage_update($stage_id,$stage_update);
	...
	public function milestone_delete($did,$stage_no,$mid){
		$this->potential_stage_model->milestone_delete($mid);
		// redirect to project list page
		redirect('potential_stage/stage_info/'.$did.'/'.$stage_no);
	}
   ...
   function stage_info($did=0, $sid=0){
        ...
        $development = $this->potential_stage_model->get_development_detail($did)->row();
        
        $stage = $this->potential_stage_model->get_stage_detail($did,$sid)->row();
		$data['stage_detail'] = $stage;

		//$data['stage_feature_photo'] = $this->potential_stage_model->get_stage_feature_photo($stage->fid)->row();
		$data['stage_feature_photos'] = $this->potential_stage_model->get_stage_feature_photo_new($did,$sid)->result();
        //$emp = $this->employee_profile_model->emp_load($salary->eid);

		$stage_milestone = $this->potential_stage_model->get_stage_milestone_detail($did,$sid)->row();
		...
		$this->table->set_empty("");
			...
			$data['stage_sub_sidebar']=$this->load->view('potential_stage/stage_sub_sidebar',$data,true);  
            $data['stage_content']=$this->load->view('potential_stage/stage_detail',$data,true);
		  
		  	$this->load->view('includes/header', $data);
		  	$this->load->view('potential_stage/stage_sidebar',$data);
		  	$this->load->view('potential_stage/stage_home',$data);
		  	$this->load->view('includes/footer', $data);
    }
    
	public function stage_documents($pid=0, $sid=0){
		$development = $this->potential_stage_model->get_development_detail($pid)->row();
		$data['title'] = $development->development_name;

		$data['number_of_stages'] = $this->potential_stage_model->get_development_number_of_stage($pid);
        $data['development_id']=$pid;
        $data['stage_id']=$sid;
        
        
        $data['documents'] = $this->potential_stage_model->getStageDocuments($pid, $sid)->result();
              
        $data['stage_documents'] = $this->potential_stage_model->get_stage_document_details($pid, $sid)->result();        
		       
		$data['stage_notes'] = $this->potential_stage_model->get_stage_note_detail($pid, $sid)->result();

        $data['stage_sub_sidebar']=$this->load->view('potential_stage/stage_sub_sidebar',$data,true);  
       	$data['stage_content']=$this->load->view('potential_stage/stage_documents',$data,true);
	...
	public function stage_documents_bycategory($pid=0, $sid=0, $cid=0){
        ...
        $data['documents'] = $this->potential_stage_model->get_stage_documents_bycategory($pid, $sid, $cid)->result();
        $data['stage_documents'] = $this->potential_stage_model->get_others_stage_documents_bycategory($pid, $sid, $cid)->result();
	...
	public function save_stage_document($pid=0, $sid=0){
        ...
        if ($this->upload->do_upload('upload_document')){
            ...
            $this->potential_stage_model->stage_document_insert($document);  
			...
			if(!empty($notify_user))
			{
				...
				$notify_user_email = $this->potential_stage_model->notify_one_user_info($notify_user);
				$notify_user_to = $notify_user_email->email;
				$notify_user_name = $notify_user_email->username;

				$dev_info = $this->potential_stage_model->get_development_detail($pid);
		        ...
		        $message .= " To view this conversation, follow this link: ".base_url()."potential_stage/stage_documents/".$pid."/".$sid;
				$message .= "</body></html>";	
        	...
        	redirect('potential_stage/stage_documents/'.$pid.'/'.$sid.'?error=1', 'refresh'); 
        } 
        redirect('potential_stage/stage_documents/'.$pid.'/'.$sid, 'refresh'); 
    }
    
	public function stage_document_detail($document_id){
        
        $stage_document_detail = $this->potential_stage_model->get_document_detail($document_id)->row();
    ...
    public function stage_photos($pid=0, $sid=0){
		...
		$data['photos'] = $this->potential_stage_model->getStagePhotos($pid, $sid)->result();
        
        $data['number_of_stages'] = $this->potential_stage_model->get_development_number_of_stage($pid);
        
        $data['stage_sub_sidebar']=$this->load->view('potential_stage/stage_sub_sidebar',$data,true); 
        $data['stage_content']=$this->load->view('potential_stage/stage_photos',$data,true);
	...
	public function stage_archive_photos($pid=0, $sid=0){
		...
		$data['photos'] = $this->potential_stage_model->getStageArchivePhotos($pid, $sid)->result();
        
        $data['number_of_stages'] = $this->potential_stage_model->get_development_number_of_stage($pid);
        
        $data['stage_sub_sidebar']=$this->load->view('potential_stage/stage_sub_sidebar',$data,true); 
        $data['stage_content']=$this->load->view('potential_stage/stage_archive_photos',$data,true);
    ...
    public function upload_stage_photo($pid=0, $sid=0){
        ...
        $user_id =$user->uid; 
         ...
         if ($this->upload->do_upload('photoimg')){
            ...
            $photo_insert_id = $this->potential_stage_model->stage_photo_insert($document); 
    ...
    public function save_stage_photo($pid=0, $sid=0){
        ...
        $this->potential_stage_model->save_stage_photo_info($photo_insert_id, $photo_info);       
              
        redirect('potential_stage/stage_photos/'.$pid.'/'.$sid, 'refresh');          
    ...
    public function stage_overview($pid=0, $sid=0){
        ...
        $data['task_info'] = $this->potential_stage_model->get_development_stage_task_list($pid,$sid)->result();
        $data['phase_info'] = $this->potential_stage_model->get_development_stage_phase_list($pid,$sid)->result();
         
        
        $data['stage_sub_sidebar']=$this->load->view('potential_stage/stage_sub_sidebar',$data,true); 
        $data['stage_content']=$this->load->view('potential_stage/stage_overview',$data,true);
    ...
    public function project_plan($pid=0, $sid=0){
        ...
        $data['stage_sub_sidebar']=$this->load->view('potential_stage/stage_sub_sidebar',$data,true); 
        $data['stage_content']=$this->load->view('potential_stage/project_plan',$data,true);
    ...
    public function plan_vs_actual($pid=0, $sid=0){
        ...
        $data['stage_sub_sidebar']=$this->load->view('potential_stage/stage_sub_sidebar',$data,true); 
        $data['stage_content']=$this->load->view('potential_stage/plan_vs_actual',$data,true);
    ...
    public function phases_list($pid=0, $sid=0){
        ...
        $data['development_id']=$pid;
        $data['stage_id']=$sid;
        $data['stages_no'] = $this->potential_stage_model->get_stage_list($pid)->result();

		$data['phase_info'] = $this->potential_stage_model->get_development_stage_phase_list($pid,$sid)->result();
 

         //$data['number_of_stages'] = $this->potential_stage_model->get_stage_list($pid)->result();
        //$data['phase_info'] = $this->project->model->get_phase_info($pid)->result();
        $data['number_of_stages'] = $this->potential_stage_model->get_development_number_of_stage($pid);
        
        $data['stage_sub_sidebar']=$this->load->view('potential_stage/stage_sub_sidebar',$data,true); 
        $data['stage_content']=$this->load->view('potential_stage/stage_phases',$data,true);
...
public function print_stage($pid=0, $sid=0)
{
    ...
    $stage_info = $this->potential_stage_model->get_stage_detail($pid)->row();       

    $data['title'] = 'Stage'.$sid;  


  $this->load->view('potential_stage/stage_print',$data);
 ...
 public function email_stage($pid=0, $sid=0){
    ...
    public function stage_infromation($tid){
        ...
        $stage = $this->potential_stage_model->get_stage_details($tid)->row();  

		$feature_photo_id= $stage->fid;
    	$feature_photo= $this->potential_stage_model->get_stage_feature_photo($feature_photo_id)->row();
	...
	public function update_task_status($task_id,$status)
	{
		...
		$this->potential_stage_model->update_status($task_id,$status);
	}
    ...
    public function stage_print($sid)
	{
	    ...
	    $stage = $this->potential_stage_model->get_stage_details($sid)->row(); 
	
	    $feature_photo_id= $stage->fid;
	    $data['feature_photo'] = $this->potential_stage_model->get_stage_feature_photo($feature_photo_id)->row();
	...
	public function send_stage_photo_message()
	{
		...
		$photo_detail= $this->potential_stage_model->get_photo_author($photo_id)->row(); 
		...
		redirect('potential_stage/stage_photos/'.$photo_dev_id.'/'.$stage_no.'?sent_email='.$email_send);
	...
	public function pdf_stage_photo($photo_id){
        ...
        $photo = $this->potential_stage_model->getStagePhotoDetail($photo_id);   
	...
	public function print_stage_photo($photo_id)
	{
	    ...
	    $data['table'] = $this->table->generate();
	  	...
	  	$this->load->view('potential_stage/stage_photo_print',$data);
	...
	public function send_stage_note_message($did)
	{
		...
		$dev_detail= $this->potential_stage_model->get_development_detail($did)->row();

		$note_detail= $this->potential_stage_model->get_note_author($note_id)->row(); 
		...
		//echo $this->email->print_debugger();
		redirect('potential_stage/stage_notes/'.$did.'/'.$stage_no.'?sent_email='.$email_send);
	...
	public function stage_task_start_date_update($task_id){
        ...
        $this->potential_stage_model->stage_task_start_date_update($task_id,$task_data);        
        
        redirect('potential_stage/phases_list/'.$dev_id.'/'.$stage_no);
	...
	public function stage_task_actual_date_update($task_id){
        ...
        $this->potential_stage_model->stage_task_actual_date_update($task_id,$task_data);        
	...
	public function add_new_milestone(){
        ...
        if ($this->input->post('submit')){
            ...
            $this->potential_stage_model->add_new_milestone($add_new_milestone);            
	...
	public function update_milestone($dev_id, $stage_no){
        ...
        if ($this->input->post('submit')){
            ...
            $this->potential_stage_model->update_milestone($dev_id,$stage_no, $update_milestone);            
	          	                                     
		} 
        redirect('potential_stage/stage_info/'.$dev_id.'/'.$stage_no); 
    }

	
	public function update_all_task_status($development_id,$phase_id,$stage_no,$status)
	{
		$this->potential_stage_model->update_all_phase_tasks($development_id, $stage_no, $phase_id,$status);
	}

	public function email_outlook_stage($photo_id)
	{
		$this->potential_stage_model->email_outlook_stage($photo_id);
	}
	...
	public function stage_photo_delete(){
		...
		$this->potential_stage_model->stage_photo_delete($stage_photo_id);
		redirect('potential_stage/stage_photos/'.$development_id.'/'.$stage_no);
	}
	...
	public function stage_archive_photo_delete(){
		...
		$stage_photo_id = $post['stage_photo_id'];
		$this->potential_stage_model->stage_photo_delete($stage_photo_id);
		redirect('potential_stage/stage_archive_photos/'.$development_id.'/'.$stage_no);
	}
	...
	public function stage_document_delete(){
		...
		$this->potential_stage_model->stage_document_delete($stage_document_id);
		redirect('potential_stage/stage_documents/'.$development_id.'/'.$stage_no, 'refresh');
	}
    ...
    public function search_stage_notes($pid=0, $sid=0){
        ...
        $data['notes'] = $this->potential_stage_model->get_search_stage_notes($pid, $sid, $search_notes)->result();

		$data['stage_notes'] = $this->potential_stage_model->get_others_search_stage_notes($pid, $sid, $search_notes)->result();
        
        $data['stage_sub_sidebar']=$this->load->view('potential_stage/stage_sub_sidebar',$data,true); 
        $data['stage_content']=$this->load->view('potential_stage/stage_notes',$data,true);
    ...
    public function stage_notes_details($nid){
        ...
        $note_detail = $this->potential_stage_model->get_note_detail($nid)->row();
        //print_r($note_detail);
    ...
    public function save_stage_note($pid=0, $sid=0){
        ...
        $data['development_id']=$pid;
        $data['stage_id']=$sid;
        //$data['notes'] = $this->potential_stage_model->get_stage_notes($pid)->result();
        ...
        $this->potential_stage_model->insert_stage_note($note_data);        
        
        redirect('potential_stage/stage_notes/'.$pid.'/'.$sid, 'refresh');
    ...
    public function stage_notes($pid=0, $sid=0){
        ...
        $data['development_id']=$pid;
        $data['stage_id']=$sid;
        $data['notes'] = $this->potential_stage_model->get_stage_notes($pid, $sid)->result();
        $data['number_of_stages'] = $this->potential_stage_model->get_development_number_of_stage($pid);
        $data['stage_notes'] = $this->potential_stage_model->get_stage_note_detail($pid, $sid)->result();
    ...
    public function notes($dev_id='', $stage_id='')
  	{
       ...
       $development = $this->potential_stage_model->get_development_detail($dev_id)->row();
       $data['title'] = $development->development_name; 

      $data['development_id']=$dev_id;
      $data['stage_id']=$stage_id;
      $data['number_of_stages'] = $this->potential_stage_model->get_development_number_of_stage($dev_id);
      $data['request_info']= $this->potential_stage_model->getStageInfo($dev_id, $stage_id);
      if($user_role==3){
      	$prev_notes = $this->potential_stage_model->getPriviousStageNotesContractor($dev_id, $stage_id);
	  }else{
		$prev_notes = $this->potential_stage_model->getPriviousStageNotes($dev_id, $stage_id);
	  }
      
      $data['stage_feature_photos'] = $this->potential_stage_model->get_stage_feature_photo_new($dev_id,$stage_id)->result();

      $data['prev_notes'] = $this->notes_image_tmpl($prev_notes);
      
      
      $data['stage_sub_sidebar']=$this->load->view('potential_stage/stage_sub_sidebar',$data,true); 
      $data['stage_content']=$this->load->view('potential_stage/stage_notes_view',$data,true);
	...
	public function photo_notes($photo_id='', $dev_id='', $stage_id='')
  	{
      	...
      	$data['development_id']=$dev_id;
      	$data['stage_id']=$stage_id;
		$data['photo'] = $this->potential_stage_model->getStagePhoto($photo_id)->row();

      	$data['number_of_stages'] = $this->potential_stage_model->get_development_number_of_stage($dev_id);
      	$data['request_info']= $this->potential_stage_model->getStageInfo($dev_id, $stage_id);
      
      	$prev_notes = $this->potential_stage_model->getPriviousStagePhotoNotes($dev_id, $stage_id);
      
      	$data['prev_notes'] = $this->notes_image_tmpl($prev_notes);
      
      
      	$data['stage_sub_sidebar']=$this->load->view('potential_stage/stage_sub_sidebar',$data,true); 
      	$data['stage_content']=$this->load->view('potential_stage/stage_photo_notes_view',$data,true);
	...
	public function show_photo_notes_with_image($rid){ 
      ...
      $prev_notes= $this->potential_stage_model->getPriviousStagephotoNotes($rid);
      ...
      if($user_role==3){
      	$prev_notes= $this->potential_stage_model->getPriviousStageNotesContractor($rid, $sid);
	  }else{
		$prev_notes= $this->potential_stage_model->getPriviousStageNotes($rid, $sid);
	  }
      ...
      $tmpl='';
       foreach ($prev_notes as $notes) {
        ...
        {
           $showuser= 'Me'; 
           $notified_user= $this->potential_stage_model->getNotifiedUserName($notes->notify_user_id);
  ...
  public function show_notes($rid, $sid, $notify_user_id=''){
	...
	$note_mail = $note7;
      ...
      $dev_detail= $this->potential_stage_model->get_development_detail($rid)->row();
      $development_name= $dev_detail->development_name;

      $request_info= $this->potential_stage_model->getStageInfo($rid, $sid);      
      $stage_name= $request_info->stage_name;      
      $request_created_by =$request_info->created_by;

		$insert_note = $this->potential_stage_model->insertNote($rid, $sid, $note_body, $user_id, $notify_user_id, $now, $private);      
        $prev_notes= $this->potential_stage_model->getPriviousStageNotes($rid, $sid);
        echo $this->notes_image_tmpl($prev_notes); 
        
      
    
        $notify_user_info=$this->potential_stage_model->get_user_info($notify_user_id);   
        ...
        foreach ($notify_user_info as $user_info) {
            ...
            $message2 .= " To view this conversation, follow this link: ".base_url()."potential_stage/notes/".$rid."/".$sid;
            $message2 .= "</body></html>";	
	...
	public function notes_delete($pid, $sid, $noteid){ 
      	...
      	$this->potential_stage_model->deleteStageNotes($noteid);
        $prev_notes= $this->potential_stage_model->getPriviousStageNotes($pid, $sid);
      	echo $this->notes_image_tmpl($prev_notes);
	...
	public function stage_phase_delete($phase_id){
		...
		$this->potential_stage_model->stage_phase_delete($phase_id);
		// redirect to Employee list page
		echo $url.'/'.$development_id.'/'.$stage_no;
		//redirect('potential_stage/'.$url.'/'.$development_id.'/'.$stage_no);
	}	
	...
	public function stage_task_delete($task_id){
		...
		$this->potential_stage_model->stage_task_delete($task_id);
		echo $url.'/'.$development_id.'/'.$stage_no.'/'.$phase_id;
		//redirect('potential_stage/'.$url.'/'.$development_id.'/'.$stage_no.'/'.$phase_id);
	}	
	...
	public function stage_task_update($task_id) {  
        ...
        if ( $this->input->post('submit')) {
		    ...
		    $this->potential_stage_model->stage_task_update($task_id,$stage_task_update);
			...
			if(!empty($notify_user))
			{
				...
				$dev_info = $this->potential_stage_model->get_development_detail($development_id)->row();
				$dev_name = $dev_info->development_name;

				$phase_info = $this->potential_stage_model->getStagePhaseInfo($phase_id);
		        ...
		        $message .= "To view this allocation, follow this link: ".base_url()."potential_stage/phases_list/".$development_id."/".$stage_no."/".$phase_id;
				$message .= "</body></html>";	
	...
	public function stage_phase_update($phase_id) {        
        ...
        if ( $this->input->post('submit')) {
		    ...
		    $this->potential_stage_model->stage_phase_update($phase_id,$stage_phase_update);
	...
	public function stage_phase_update_old($phase_id) {        
        ...
        if ( $this->input->post('submit')) {
					...
					foreach($results as $result){
						if($result->ordering >= $ordering){
							...
							if(!empty($dependency_id)){
								...
								$this->potential_stage_model->stage_phase_update($dependency_id,$stage_phase_update_dependency);
								...
								foreach($results1 as $result){
									...
									$this->potential_stage_model->stage_task_update($task_id,$stage_task_update_dependency);
								}
							}
						}
					}
			...
			if(!empty($post['dependency'])){
				$this->potential_stage_model->stage_phase_dependency_delete($phase_id);
				...
				$this->potential_stage_model->stage_phase_dependency($stage_phase_dependency);
			}else{
				$this->potential_stage_model->stage_phase_dependency_delete($phase_id);
			}
	...
	public function stage_phase_dependency_name_load($id) {
		$this->potential_stage_model->stage_phase_dependency_name_load($id);
	}
	...
	public function stage_phase_add() {  
        ...
        if ( $this->input->post('submit')) {
		    ...
		    $phase_id = $this->potential_stage_model->stage_phase_add($stage_phase_add);
	...
	public function stage_task_add() { 
        ...
        if ( $this->input->post('submit')) {
		    ...
		    $this->potential_stage_model->stage_task_add($stage_task_add);
	...
	public function photo_action_featured($photo_id, $checked_val){
		...
		$this->potential_stage_model->update_photo_featured($photo_id, $checked_val);
	}
	public function photo_action_private($photo_id, $checked_val){
		
		$this->potential_stage_model->update_photo_private($photo_id, $checked_val);
	}
	public function insert_photo_notes($rid, $notify_user_id){
      ...
      $request_created_by =$request_info->created_by;
		...
		$insert_note = $this->potential_stage_model->insertPhotoNote($rid, $sid, $note_body, $user_id, $notify_user_id, $now);      
        $prev_notes= $this->potential_stage_model->getPriviousStagePhotoNotes($rid, $sid);
        echo $this->notes_image_tmpl($prev_notes); 
	...
	public function photo_notes_delete($pid, $noteid){ 
      ...
      $this->potential_stage_model->deleteStagephotoNotes($noteid);
      $prev_notes= $this->potential_stage_model->getPriviousStagephotoNotes($pid);
      echo $this->notes_image_tmpl($prev_notes);
	...
	public function stage_document_update($id){
        ...
        if ($this->input->post('submit')){
            ...
            $this->potential_stage_model->stage_document_update($id,$update);            
