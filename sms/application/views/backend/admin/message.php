...
<div class="mail-env">
    ...
    <div class="mail-sidebar" style="min-height: 800px;">
        ...
        <ul class="mail-menu">
            ...
            <?php
            $current_user = $this->session->userdata('login_type') . '-' . $this->session->userdata('user')->uid;

			$this->db->where('company_id', $this->session->userdata('user')->company_id);
            $this->db->where('sender', $current_user);
            $this->db->or_where('reciever', $current_user);
            $message_threads = $this->db->get('sms_message_thread')->result_array();
            foreach ($message_threads as $row):
                ...
                <li class="<?php if (isset($current_message_thread_code) && $current_message_thread_code == $row['message_thread_code']) echo 'active'; ?>">
                    <a href="<?php echo base_url(); ?>admin/message/message_read/<?php echo $row['message_thread_code']; ?>" style="padding:12px;">
						...
						<?php echo $this->db->get_where('users', array('uid' => $user_to_show_id))->row()->username; ?>
						<?php }elseif($user_to_show_type=='student'){ ?>
						<?php echo $this->db->get_where('sms_student', array('student_id' => $user_to_show_id))->row()->name; ?>
						<?php }elseif($user_to_show_type=='teacher'){ ?>
						<?php echo $this->db->get_where('sms_teacher', array('teacher_id' => $user_to_show_id))->row()->name; ?>
						<?php } ?>
