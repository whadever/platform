...
class Sms_model extends CI_Model {
    ...
    function send_sms_via_clickatell($message = '' , $reciever_phone = '') {
        ...
        $clickatell_user       = $this->db->get_where('sms_settings', array('type' => 'clickatell_user'))->row()->description;

        $clickatell_password   = $this->db->get_where('sms_settings', array('type' => 'clickatell_password'))->row()->description;

        $clickatell_api_id     = $this->db->get_where('sms_settings', array('type' => 'clickatell_api_id'))->row()->description;
    ...
    function send_sms_via_twilio($message = '' , $reciever_phone = '') {
        ...
        $account_sid    = $this->db->get_where('sms_settings', array('type' => 'twilio_account_sid'))->row()->description;

        $auth_token     = $this->db->get_where('sms_settings', array('type' => 'twilio_auth_token'))->row()->description;
        ...
        $client->account->messages->create(array( 
            ...
            'From'      => $this->db->get_where('sms_settings', array('type' => 'twilio_sender_phone_number'))->row()->description,
