...
if ( ! function_exists('get_phrase'))
{
	function get_phrase($phrase = '') {
		$CI	=&	get_instance();
		$CI->load->database();
		$current_language	=	$CI->db->get_where('sms_settings' , array('type' => 'language'))->row()->description;
		...
		$check_phrase	=	$CI->db->get_where('sms_language' , array('phrase' => $phrase))->row()->phrase;
		if ( $check_phrase	!=		$phrase)
			$CI->db->insert('sms_language' , array('phrase' => $phrase));
			
		
		// query for finding the phrase from `language` table
		$query	=	$CI->db->get_where('sms_language' , array('phrase' => $phrase));
		$row   	=	$query->row();	
