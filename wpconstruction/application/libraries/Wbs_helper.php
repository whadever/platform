...
//                
    ...
    function log($page, $message){
        $ci =& get_instance();
        $user = $ci->session->userdata('user');
        $ci->db->insert('construction_user_logs',array('user_id'=>$user->uid,'page'=>$page,'message'=>$message));
    }
    
    function is_own_job($jid){
        $ci =& get_instance();
        $user = $ci->session->userdata('user');
        if($user && $jid){
            $row = $ci->db->get_where('construction_development',array('id'=>$jid, 'wp_company_id' => $user->company_id))->row();
            if($row){
                return;
            }
        }
