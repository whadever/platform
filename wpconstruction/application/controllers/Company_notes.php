...
class Company_Notes extends CI_Controller {
  ...
  public function show_notes($cid, $notify_user_id=''){
	...
	$message2= '';
        $message2 .= '<html><body>';	
	$message2 .= "Hello, <strong>".$notes_from."</strong> has added a new note on project '".$company_name."'<br />";
        $message2 .= "Note Description: " . $note_body . " <br />";
        $message2 .= " To view this conversation, follow this link: http://williamscorporation.co.nz/tms/project_notes/index/".$cid."";
	$message2 .= "</body></html>";	
      ...
      foreach ($prev_notes as $notes) {
        ...
        if($notes->notes_by==$user_id)
        {
           ...
           if(!$notes->notes_image_id == null){
               ...
               $tmpl .= '<div class="'.$align_class.'"><div class="userme"> :'.$showuser.'</div> <div style="float: right;" class="notes_body"><img height="100" width="100" src="'.base_url().'uploads/notes/images/'.$show_file->filename.'"/></div><div style="float:right;"><span class="time-left1">'.$creation_time.'</span></div> </div>';
           }else{
           ...
           $tmpl .= '<div class="clear"></div>';
            ...
            if(!$notes->notes_image_id == null){
                $show_file= $this->company_notes_model->getNotesImage($notes->notes_image_id);
                $tmpl .= '<div class="'.$align_class.'"><div class="useranother">'.$showuser.':</div> <div style="float: left;" class="notes_body"><img height="100" width="100" src="'.base_url().'uploads/notes/images/'.$show_file->filename.'"/></div><div style="float:left;"><span class="time-right1">'.$creation_time.'</span></div> </div>';
            }
            else{
