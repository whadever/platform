...
class User extends CI_Controller {  
	...
	public function user_auto_login()
    {
		if($this->input->post('login')){
			...
			if($result) {
                ...
                $user = $this->session->userdata('user');
				$result1=$this->User_model->check_app_per($user->uid);
				if($result1){
					redirect("constructions", 'refresh');
				}else {
    ...
    public function user_logout()
    {
		...
		$user=  $this->session->userdata('user');
		$user_id = $user->uid; 
		$job_id = $_SESSION[$_SERVER['SERVER_NAME']]['current_job'];

		foreach($_COOKIE as $key => $value) {
			setcookie($key,$value,TIME()-10000,"/");
		}
		unset($_SESSION[$_SERVER['SERVER_NAME']]);
		$this->User_model->insert_job_id($job_id ,$user_id);
