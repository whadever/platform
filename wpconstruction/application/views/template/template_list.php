...
function setprojectid(pid)
{
	 ...
	 document.getElementById('clone-template').href=base_url+'constructions/clone_template/'+pid;
}
</script>
			...
			<table class="table-hover">
				...
				<?php foreach ($templates as $template){ ?>
					<tr class="check" onclick="<?php echo 'setprojectid('. $template->id . ');'; ?>">
						...
						<!-- <td><?php if($template->template_type == 1) { echo 'Job'; } else { echo 'Stage'; } ?></td> -->
						<td><?php echo $this->wbs_helper->to_report_date($template->created); ?></td>
						<td>
						<?php 
						$phase_query = $this->db->query("SELECT * FROM construction_template_phase where template_id=".$template->id);
						...
						<td>
						<?php 
						$task_query = $this->db->query("SELECT * FROM construction_template_task where template_id=".$template->id);
