...
class Template_model extends CI_Model {
	...
	public function save_phase_and_task($template_id,$no_phase,$no_task){
		...
		{
			$array_phase[] = array('template_id' => $template_id, 'phase_no' => $i, 'phase_name' => 'Phase Name', 'ordering' => $i-1);
		}
    ...
    public function template_list() {
    	...
    	$query = $this->db->query("SELECT temp.*, users.username FROM construction_template temp LEFT JOIN users ON temp.created_by = users.uid WHERE wp_company_id = {$wp_company_id} ORDER BY temp.`id` DESC");
        return $query;
	} 
	...
	function template_phase_delete($phase_id,$template_id){
		$phase = $this->db->query("select phase_no from {$this->table_template_phase} where id = {$phase_id}")->row();
		...
		$sql = "update ".$this->table_template_phase." set phase_no = phase_no - 1 where phase_no > {$phase->phase_no}  and template_id = {$template_id}";
		$this->db->query($sql);

		$sql = "update ".$this->table_template_task." set phase_no = phase_no - 1 where phase_no > {$phase->phase_no}  and template_id = {$template_id}";
		$this->db->query($sql);

		$sql = "update construction_development_phase set phase_no = phase_no - 1 where phase_no > {$phase->phase_no}  and template_id = {$template_id}";
		$this->db->query($sql);
	}
	...
	function template_phase_ordering(){
		...
		foreach ($_POST['listItemPhase'] as $position => $item){
                    ...
                    $sql = "UPDATE construction_template_phase SET ordering=$position WHERE id=$item";
                    //$res = mysql_query($sql);
                    $this->db->simple_query($sql);
                } 
                /* the first phase will not have any dependency */
                $this->db->simple_query("UPDATE construction_template_phase SET dependency=NULL WHERE id=$first_item");
                return $res;
	}
	...
	function template_task_ordering(){
		...
		foreach ($_POST['listItemTask'] as $position => $item){
            $sql = "UPDATE template_task SET ordering=$position WHERE id=$item";
            $res = mysql_query($sql);
        }
        return $res;
	}
