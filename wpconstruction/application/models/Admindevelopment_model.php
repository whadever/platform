...
class AdminDevelopment_model extends CI_Model {
	...
	private $table_template = 'construction_template';
	private $table_template_phase = 'construction_template_phase';
	private $table_template_task = 'construction_template_task';
	private $table_file = 'construction_file';
	private $table_dev_phase = 'construction_development_phase';
	private $table_dev_task = 'construction_development_task';
    ...
    public function development_list() {
		...
		$sql = "SELECT dev1.*, dev2.development_name parent_unit_name, ct1.template_name construction_template, ct2.template_name pre_construction_template
				FROM construction_development dev1 LEFT JOIN construction_template ct1 ON dev1.tid = ct1.id
					LEFT JOIN construction_template ct2 on dev1.pre_construction_tid = ct2.id
					LEFT JOIN construction_development dev2 on dev1.parent_unit = dev2.id
					WHERE dev1.wp_company_id='$wp_company_id' ORDER BY dev1.`id` DESC";
	...
	function development_choice_template(){
        $query = $this->db->query("SELECT tep.`template_name`, tep.`id` FROM construction_template tep where template_type=1 ORDER BY tep.`id` DESC");
        $rows = array();
	...
	function stage_choice_template(){
        $query = $this->db->query("SELECT tep.`template_name`, tep.`id` FROM construction_template tep where template_type=2 ORDER BY tep.`id` DESC");
        $rows = array();
        foreach ($query->result() as $row){
            $rows[$row->id] = $row->template_name; 
        }
	...
	function development_template_update($did,$tid,$construction_phase){
		...
		$this->db->select('template_id, phase_no, phase_name, phase_length, ordering, phase_person_responsible' );
		$this->db->where('template_id', $tid);
		$template_phase_data = $this->db->get($this->table_template_phase)->result();

		//print_r($template_phase_data);

		foreach($template_phase_data as $tdata)
		{
			$array[] = array('development_id' => $did, 'template_id' => $tdata->template_id, 'phase_no' => $tdata->phase_no, 'phase_name' => $tdata->phase_name, 'phase_length' => $tdata->phase_length, 'ordering' => $tdata->ordering, 'phase_person_responsible' => $tdata->phase_person_responsible, 'construction_phase' => $construction_phase);
			//$this->db->set($array);
		...
		foreach($phase_ids as $phase_id)
		{
			...
			$this->db->select('id, template_id, task_name, task_length, task_person_responsible, phase_no, ordering, task_company, task_category' );
			$this->db->where('template_id', $tid);
			...
			foreach($template_task_data as $tdata)
			{
				...
				$array_task[] = array('development_id' => $did, 'template_id' => $tdata->template_id, 'phase_id' => $phase_id->id, 'task_name' => $tdata->task_name, 'task_length' => $tdata->task_length, 'task_person_responsible' =>$tdata->task_person_responsible, 'ordering' => $tdata->ordering , 'construction_phase' => $construction_phase,'construction_template_task_id'=>$tdata->id, 'task_category' => $tdata->task_category, 'task_company' => $tdata->task_company);
				//$this->db->set($array);
				//$this->db->insert('mytable');
	...
	function development_phase_ordering(){
		...
		foreach ($_POST['listItemPhase'] as $position => $item){
            $sql = "UPDATE construction_development_phase SET ordering=$position WHERE id=$item";
	...
	function development_task_ordering(){
		...
		foreach ($_POST['listItemTask'] as $position => $item){
            $sql = "UPDATE construction_development_task SET ordering=$position WHERE id=$item";
	...
	function stage_phase_ordering(){
		//$list $_GET['listItem_'.$phase_id];
		foreach ($_POST['listItemPhase'] as $position => $item){
            $sql = "UPDATE construction_stage_phase SET ordering=$position WHERE id=$item";
            $res = mysql_query($sql);
        }
        return $res;
	}
	...
	function stage_task_ordering(){
		//$list $_GET['listItem_'.$phase_id];
		foreach ($_POST['listItemTask'] as $position => $item){
            $sql = "UPDATE construction_stage_task SET ordering=$position WHERE id=$item";
            $res = mysql_query($sql);
        }
        return $res;
	}
