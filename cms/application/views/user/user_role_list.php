...
function move_keyword(group_id)
{
	
	kyeword = $('#keyword_' + group_id).val();
	...
	if(kyeword==''){
		...
		$.ajax({
			url: window.Url + 'user/keyword_permission/'+ group_id + '/' + encodeURIComponent(kyeword),
			type: 'GET',
			success: function(data) 
			{
				$('#keyword_' + group_id).val(''); 
			    appendhtml = '<div id="sigle_keyword_' + data + '">'+ kyeword + '<span style="float:right;padding:0px 10px;" onclick="keyworddelete('+data+')"><img src="'+window.Url+'images/icon/delete_user.png" height="22" /></span></div>';
				$('#keyword_load_list_' + group_id).prepend(appendhtml);  	   
			},      
		});
...
function keyworddelete(id)
{
	...
	if (r==true) {
		$.ajax({          
			url: window.Url + 'user/keyword_permission_delete/' + id,
			type: 'GET',
			success: function(data) 
			{
	 			$('#sigle_keyword_' + id).remove(); 
			},
		});
	}	
}
...
$(document).ready(function () {
	...
	$('#permission_id').on('change', function(){ 
		id = $(this).val();  
		newurl = window.Url + 'user/user_role_list?permission_id=' + id;
	...
	<div class="clr">
		<div class="cms_tabs">
			<table style="width:80%; margin:0px auto; border: none;">
				<tr>
					<td class="active" align="center"><a class="cmstabs" href="<?php echo base_url();?>user/user_role_list">Permissions</a></td>
	...
	<div class="clr">
		<div class="select-permission">
			<select name="permission_id" id="permission_id">
				...
				<?php
				$load_groups = $ci->user_model->groups_load()->result();
				foreach($load_groups as $load_group){
					if($_GET['permission_id']==$load_group->id){
						$selected = 'selected';
					}else{
						$selected = '';
					}
					echo '<option '.$selected.' value="'.$load_group->id.'">'.$load_group->group_name.'</option>';
				}
				?>
			</select>
		</div>

		<div class="cms_icons icon_role">
...
<div id="AddNewGroup" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		...
		$group_info = $this->user_model->user_group_list($_GET['permission_id'])->result();
		...
		?>
			...
			<div class="group_content">
				...
				<div class="left_permission_box">
					...
					<h3 class="txtcntr">Keyword</h3>
					<div class="permission_box" style="height: 31px;">
						<div id="keyword_list_<?php echo $group_info[$i]->id; ?>" class="keyword_list permission_box_wapper">
							<input style="width: 330px;border: none;" name="keyword" id="keyword_<?php echo $group_info[$i]->id; ?>" />
						</div>
					</div>
				</div>
				...
				<div class="middle_arrow_box">
					...
					<div class="visible_hidden_arrow clr">
						...
						<div class="permission_arrow" onclick="move_keyword(<?php echo $group_info[$i]->id; ?>)"> &#8594;  </div>
						<div class="delete_group_box">
				...
				<div class="right_permission_box">
					...
					<h3 style="text-align:center">Keyword</h3>
					<div class="permission_box">
						<div id="keyword_load_list_<?php echo $group_info[$i]->id; ?>" class="keyword_load_list permission_box_wapper">
							<?php 
							$keywords = $ci->user_model->keyword_list($group_info[$i]->id)->result();
							foreach($keywords as $key)
							{
							?>
								<div id="sigle_keyword_<?php echo $key->id; ?>"><?php echo $key->keyword; ?><span style="float:right;padding:0px 10px;" onclick="keyworddelete(<?php echo $key->id; ?>)"><img src="<?php echo base_url(); ?>images/icon/delete_user.png" height="22" /></span></div>
							<?php
							}
							?>
						</div>
