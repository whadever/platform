...
class Consent_model extends CI_Model
{
	...
	public function getallcolumndes($id)
	{ 
	...
	public function get_consent_info($month_start_date,$month_last_day)
	{
		...
		$this->db->where('design_approval_date >= ', $month_start_date);
		$this->db->where('design_approval_date <= ', $month_last_day);
		$this->db->order_by('id', 'ASC');
		return $this->db->get($this->table_consent)->result();
	}
	public function send_consent_mail($job_id,$field_name,$job_no,$consent_name,$data=''){

		$user = $this->session->userdata('user');
		$wp_company_id =  $user->company_id;
		$email_status = $this->check_already_sent($job_id,$field_name);

		if(!$email_status){

			$html = '<html><body>';
		    $html .= 'Hi,<br>';
			if($field_name == 'council_chch'){
				$user_email = "pricingchch@horncastle.co.nz";
				$from_email = "Consent@wclp.co.nz";
				$subject ='New mail for pricing request';
				$html .= 'The following Turnkey in ('.$data.') Council has been approved<br>';
			    $html .= 'J number # '.$job_no.' Consent # '.$consent_name.'<br>';
				$html .= 'These approved plans have been uploaded to the GC<br>';
				$html .= 'Please enter the date in the Pricing requested column once complete to confirm your task is complete<br>';
				$html .= 'If a specidication has been made at the same time please upload to GC on add the date';
			   	$html .= "</body></html>";
				$headers = "From: ".$from_email. "\r\n";
		    	$headers .= "Reply-To: ". $from_email . "\r\n";
			}elseif($field_name == 'council_auc'){
				$user_email = "pricingakl@horncastle.co.nz";
				$from_email = "Consent@wclp.co.nz";
				$subject ='New mail for pricing request';
				$html .= 'The following Turnkey in ('.$data.') Council has been approved<br>';
			    $html .= 'J number # '.$job_no.' Consent # '.$consent_name.'<br>';
				...
				$headers = "From: ".$from_email. "\r\n";
		    	$headers .= "Reply-To: ". $from_email . "\r\n";
			}elseif($field_name == 'kitchen_disign_type_std'){
				$user_email = "interiors@horncastle.co.nz";
				$from_email = "Consent@wclp.co.nz";
				$subject ='New request mail for Kitchen Design STD type';
				$html .= 'The following Turnkey in ('.$data.') Council has been approved<br>';
			    $html .= 'J number # '.$job_no.' Consent # '.$consent_name.'<br>';
				$html .= 'These approved plans have been uploaded to the GC<br>';
				$html .= 'This House/Unit has a standard kitchen so only kitchen colours are required. Please include with the external colour sheet<br>';
				$html .= 'Please enter the date in the Kitchen design requested column once complete to confirm your task is complete<br>';
			   	$html .= "</body></html>";
				$headers = "From: ".$from_email. "\r\n";
		    	$headers .= "Reply-To: ". $from_email . "\r\n";
			}elseif($field_name == 'kitchen_disign_type_designer'){
				$user_email = "interiors@horncastle.co.nz";
				$from_email = "Consent@wclp.co.nz";
				$subject ='New request mail for Kitchen Design DESIGNER type';
				$html .= 'The following Turnkey in ('.$data.') Council has been approved<br>';
			    $html .= 'J number # '.$job_no.' Consent # '.$consent_name.'<br>';
				$html .= 'These approved plans have been uploaded to the GC.<br>';
				$html .= 'This House/Unit has a DESIGNER kitchen so please arrange for a design including colours<br>';
				$html .= 'Please enter the date in the Kitchen design requested column once signed off to confirm your task is complete<br>';
			   	$html .= "</body></html>";
				$headers = "From: ".$from_email. "\r\n";
		    	$headers .= "Reply-To: ". $from_email . "\r\n";
			}elseif($field_name == 'spec_loaded_on_gc'){
				$user_email = "interiors@horncastle.co.nz";
				$from_email = "Consent@wclp.co.nz";
				$subject ='New mail for pricing request';
				$html .= 'The following Turnkey in ('.$data.') Council has been approved<br>';
			    $html .= 'J number # '.$job_no.' Consent # '.$consent_name.'<br>';
				$html .= 'Please complete the External colours and upload to the GC<br>';
				$html .= 'Please upload to the GC and enter the date in the Spec loaded on GC column to confirm your task is complete<br>';
			   	$html .= "</body></html>";
				$headers = "From: ".$from_email. "\r\n";
		    	$headers .= "Reply-To: ". $from_email . "\r\n";
			}elseif($field_name == 'pricing_requested'){
				$user_email = "pricingakl@horncastle.co.nz";
				$from_email = "Consent@wclp.co.nz";
				$subject ='New mail for pricing request';
				$html .= 'The following Turnkey in ('.$data.') Council has been approved<br>';
				...
				$headers = "From: ".$from_email. "\r\n";
		    	$headers .= "Reply-To: ". $from_email . "\r\n";
			}elseif($field_name == 'kitchen_disign_requested'){
				$user_email = "interiors@horncastle.co.nz";
				$from_email = "Consent@wclp.co.nz";
				$subject ='New mail for kitchen design request';
				$html .= 'The following Turnkey in ('.$data.') Council has been approved<br>';
			    $html .= 'J number # '.$job_no.' Consent # '.$consent_name.'<br>';
				$html .= 'Please complete the External colours and upload to the GC<br>';
				$html .= 'Please upload to the GC and enter the date in the Spec loaded on GC column to confirm your task is complete<br>';
				...
				$headers = "From: ".$from_email. "\r\n";
		    	$headers .= "Reply-To: ". $from_email . "\r\n";
			}elseif($field_name == 'colours_requested_and_loaded_on_gc'){
				$user_email = "interiors@horncastle.co.nz";
				$from_email = "Consent@wclp.co.nz";
				$subject ='New mail for color requested and loaded on GC';
				$html .= 'The following Turnkey in ('.$data.') Council has been approved<br>';
			    $html .= 'J number # '.$job_no.' Consent # '.$consent_name.'<br>';
				$html .= 'Please complete the External colours and upload to the GC<br>';
				$html .= 'Please enter the date in the COLOUR REQUESTED column once signed off to confirm your task is complete<br>';
			   	$html .= "</body></html>";
				$headers = "From: ".$from_email. "\r\n";
		    	$headers .= "Reply-To: ". $from_email . "\r\n";
			}
			...
			if($wp_company_id == 34){
		        if(mail($user_email, $subject, $html, $headers)){
					$this->update_mail_status($job_id,$field_name);
				}
			}
		}
	}

	function check_already_sent($job_id, $field_name){
		$this->db->where('job_id',$job_id);
		$this->db->where('field_name',$field_name);
		return $this->db->get('mail_status')->row();
	}

	function update_mail_status($job_id, $field_name){
		$insert_data = array(
			'job_id' => $job_id,
			'field_name' => $field_name,
			'status' => 1
		);
		$this->db->insert('mail_status',$insert_data);
	}
	...
	public function get_consent_info_by_monthid($month_id, $parent_id=0,$keywords,$ss,$report_search_value,$start_date,$end_date)
	{
		...
		//if(($report_search_value=='' && $start_date=='' && $end_date=='') || $parent_id>0){
			...
			if($keywords!='' || $ss!='')
			{
				if($ss!='')
				{
					...
					for($i = 0; $i < count($ss); $i++){
						...
						if($ss[$i]=='design_approval_date' || $ss[$i]=='price_approved_date' || $ss[$i]=='pim_logged' 
    ...
    public function save_consent_ordering($ordering, $month_id)
	{
		...
		for($j=0; $j< count($fields); $j++)
		{
			...
			{	
				...
				$sql_update_child = "UPDATE consents SET month_id = $month_id WHERE parent_id = $item";
            	$res_update = $this->db->query($sql_update_child);
			}
