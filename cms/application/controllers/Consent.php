...
class Consent extends CI_Controller
{
	...
	{ 
		...
		$consent_data = array(
                    ...
                    'design_approval_date' =>  $approval_date,
	...
	public function submit_consent()
	{
		...
		$consent_data = array(
			...
			'design' =>$_POST['design'],
			'design_approval_date' =>  $this->wbs_helper->to_mysql_date($_POST['approval_date']),
	...
	function consent_update($field_name,$job_no,$has_child){
		...
		if( $field_name == 'design_approval_date' or 
	...
	public function show_report()
	{
		...
		$message .= '<th style="width:1%">Job No.</th>';
	 	...
	 	if($user_permission_type[0]->display_type == 1){  
			...
			if(in_array('design_approval_date',$search_per)){
		...
		for($i=0;$i<count($consent_info);$i++)
		{
			...
			if($user_permission_type[1]->display_type == 1){
				...
				}elseif($consent_info[$i]->design =='Allo'){
					$text_design = 'Allo';
					$bg_design = 'background-color:orange;color:white;';
				}
				$text_design = $consent_info[$i]->design;
				if(in_array('design',$search_per)){
			...
			if($user_permission_type[2]->display_type == 1){
				...
				if($consent_info[$i]->design_approval_date != '0000-00-00'){
				...
				if(in_array('design_approval_date',$search_per)){
					if($consent_info[$i]->design_approval_date == '0000-00-00'){
						...
						$message .= '<td style="'.$bg_design_approval_date.'">'.$this->wbs_helper->to_report_date($consent_info[$i]->design_approval_date).'</td>';
					}
				}
			...
			if($user_permission_type[48]->display_type == 1){
				...
				if($consent_info[$i]->pricing_requested == '0000-00-00' && ( $consent_info[$i]->contract_type == 'EQ' or $consent_info[$i]->contract_type == 'BC')){
					...
					$bg_pricing_requested = '';
				}elseif($consent_info[$i]->design_approval_date!='0000-00-00' && $consent_info[$i]->contract_type == 'HL' && $consent_info[$i]->design_approval_date > $twenty_days_before && $consent_info[$i]->pricing_requested =='0000-00-00'){
					$text_pricing_requested = "REQ";
					$bg_pricing_requested = 'background-color:red;color:white;';	
				}elseif($consent_info[$i]->design_approval_date < $twenty_days_before && $consent_info[$i]->design_approval_date!='0000-00-00' && $consent_info[$i]->pricing_requested =='0000-00-00'){
			...
			if($user_permission_type[49]->display_type == 1){
				...
				if($consent_info[$i]->pricing_for_approval == '0000-00-00' && ( $consent_info[$i]->contract_type == 'EQ' or $consent_info[$i]->contract_type == 'BC')){
					...
					$bg_pricing_for_approval = '';
				}elseif($consent_info[$i]->design_approval_date!='0000-00-00' && $consent_info[$i]->contract_type == 'HL' && $consent_info[$i]->pricing_requested =='0000-00-00' && $consent_info[$i]->pricing_for_approval=='0000-00-00' ){
			...
			if($user_permission_type[52]->display_type == 1){
				...
				if($consent_info[$i]->kitchen_disign_requested == '' && ( $consent_info[$i]->contract_type == 'EQ' or $consent_info[$i]->contract_type == 'BC')){		
					...
					$bg_kitchen_disign_requested = '';
				}elseif( $consent_info[$i]->ok_to_release_to_marketing == 'Yes' && $consent_info[$i]->pricing_for_approval !='0000-00-00' && $twenty_days_before < $consent_info[$i]->design_approval_date){
					$text_kitchen_disign_requested = "REQ";
					$bg_kitchen_disign_requested = "background-color:red;color:white;";	
				}elseif($twenty_days_before > $consent_info[$i]->design_approval_date && $consent_info[$i]->design_approval_date !='0000-00-00' ){
			...
			if($user_permission_type[53]->display_type == 1){
				...
				if( $consent_info[$i]->ok_to_release_to_marketing == 'Yes' && $consent_info[$i]->pricing_for_approval !='0000-00-00' && $consent_info[$i]->colours_requested_and_loaded_on_gc == '0000-00-00' && $twenty_days_before < $consent_info[$i]->design_approval_date){
			...
			if($user_permission_type[54]->display_type == 1){
				...
				if($consent_info[$i]->kitchen_disign_requested == 'REQ' OR($consent_info[$i]->ok_to_release_to_marketing == 'Yes' && $twenty_days_before < $consent_info[$i]->design_approval_date)){
			...
			for($i=0; $i<count($consent_info); $i++)
			{
				...
				if($user_permission_type[2]->display_type == 1){
					if($consent_info[$i]->design_approval_date == '0000-00-00'){
						...
						$message .= '<td>'.$this->wbs_helper->to_report_date($consent_info[$i]->design_approval_date).'</td>';
					}
				}
				if($user_permission_type[3]->display_type == 1){
					if($consent_info[$i]->pim_logged == '0000-00-00'){
			...
			for($a = 0; $a < count($segment_print_arr); $a++)
			{
				if($segment_print_arr[$a] == $p )	
				{
					...
					{										
						...
						if($user_permission_type[2]->display_type == 1)
						{
							if($consent_info[$t]->design_approval_date == '0000-00-00')
							{
								$message .= 'Approval Date : %0D%0A';
							}
							else
							{
								$message .= 'Approval Date : '.$this->wbs_helper->to_report_date($consent_info[$t]->design_approval_date).'%0D%0A';
							}
						}
				...
				if($segment_print_arr[$a] == $p )	
				{
					...
					for($t = 0; $t < count($consent_info) ; $t++)
					{
						...
						if($user_permission_type[2]->display_type == 1)
						{
							...
							{
								$message .= '<td style="width:25%">Approval Date</td><td bgcolor="#ebecec" style="width:25%">'.$this->wbs_helper->to_report_date($consent_info[$t]->design_approval_date).'</td>';
							}
						}
						$message .= "</tr>";
	...
	public function consent_list_report($id)
	{
		...
		if($user_permission_type[2]->display_type == 1)
		{
			if($consent_info->design_approval_date == '0000-00-00')
			{
				$message .= '<td style="width:40%">Approval Date</td><td style="width:60%"></td>';
			}
			else
			{
				$message .= '<td style="width:40%">Approval Date</td><td style="width:60%">'.$this->wbs_helper->to_report_date($consent_info->design_approval_date).'</td>';
			}
		}
		$message .= "</tr>";
	...
	function consent_template_delete($id){
		$this->db->where('id', $id);
		$this->db->delete('consent_template');
	}
