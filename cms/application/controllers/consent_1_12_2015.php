...
class Consent extends CI_Controller
{
	...
	function consent_update($field_name,$job_no,$has_child){
		...
		$this->consent_model->consent_update_child($update_data, $job_no,$has_child,$parent_id);
	...
	public function show_report()
	{
		...
		for($i=0;$i<count($consent_info);$i++)
		{
			...
			if($user_permission_type[0]->display_type == 1){
				...
				$message .= '<td>'.$consent_info[$i]->pre_construction_sign.'</td>';
			} if($user_permission_type[1]->display_type == 1){
				$design_bg = '';
				if($consent_info[$i]->design == 'REQ Brief'){$design_bg = 'style="background-color:yellow"';}
				if($consent_info[$i]->design == 'Brief'){$design_bg = 'style="background-color:blue"';}
				if($consent_info[$i]->design == 'Hold'){$design_bg = 'style="background-color:red;color:white"';}
				if($consent_info[$i]->design == 'Sign'){$design_bg = 'style="background-color:orange"';}
				if($consent_info[$i]->design == 'Consent'){$design_bg = 'style="background-color:#90ee90"';}
				$message .= '<td '.$design_bg.'">'.$consent_info[$i]->design.'</td>';
				...
				$message .= '<td>'.$consent_info[$i]->ok_to_release_to_marketing.'</td>';
			} if($user_permission_type[48]->display_type == 1){
				$message .= '<td>'.$consent_info[$i]->pricing_requested.'</td>';
			} if($user_permission_type[49]->display_type == 1){
				$message .= '<td>'.$consent_info[$i]->pricing_for_approval.'</td>';
			} if($user_permission_type[37]->display_type == 1){

				if($consent_info[$i]->price_approved_date == '0000-00-00'){
					$message .= '<td>&nbsp;</td>';
				}else{
					$message .= '<td style="background-color:#90ee90; color:white;">'.$this->wbs_helper->to_report_date($consent_info[$i]->price_approved_date).'</td>';
				}

			} if($user_permission_type[50]->display_type == 1){
				$message .= '<td>'.$consent_info[$i]->approved_for_sale_price.'</td>';
			} if($user_permission_type[51]->display_type == 1){
				$message .= '<td>'.$consent_info[$i]->kitchen_disign_type.'</td>';
			} if($user_permission_type[52]->display_type == 1){
				$message .= '<td>'.$consent_info[$i]->kitchen_disign_requested.'</td>';
			} if($user_permission_type[53]->display_type == 1){
				$message .= '<td>'.$consent_info[$i]->colours_requested_and_loaded_on_gc.'</td>';
			} if($user_permission_type[54]->display_type == 1){
				$message .= '<td>'.$consent_info[$i]->kitchen_design_loaded_on_gc.'</td>';
			} if($user_permission_type[55]->display_type == 1){
				$message .= '<td>'.$consent_info[$i]->developer_colour_sheet_created.'</td>';
			} if($user_permission_type[56]->display_type == 1){
				$message .= '<td>'.$consent_info[$i]->spec_loaded_on_gc.'</td>';
			} if($user_permission_type[57]->display_type == 1){
				$message .= '<td>'.$consent_info[$i]->loaded_on_intranet.'</td>';
			} if($user_permission_type[58]->display_type == 1){
				$message .= '<td>'.$consent_info[$i]->website.'</td>';
			} if($user_permission_type[59]->display_type == 1){
				$message .= '<td>'.$consent_info[$i]->render_requested.'</td>';
			} if($user_permission_type[60]->display_type == 1){
				$message .= '<td>'.$consent_info[$i]->render_received.'</td>';
			} if($user_permission_type[61]->display_type == 1){
				$message .= '<td>'.$consent_info[$i]->brochure.'</td>';
			} if($user_permission_type[3]->display_type == 1){
			...
			if($user_permission_type[30]->display_type == 1){
				...
				$order_site_levels_bg = '';
				if($consent_info[$i]->order_site_levels == '') {$order_site_levels_bg='style="background-color:#FFFFFF; color:black"';} 
				if($consent_info[$i]->order_site_levels == 'Received') {$order_site_levels_bg='style="background-color:#90ee90"';}
				if($consent_info[$i]->order_site_levels == 'Sent') {$order_site_levels_bg='style="background-color:#70B5FF"';}
				$message .= '<td '.$order_site_levels_bg.'>'.$consent_info[$i]->order_site_levels.'</td>';

			} if($user_permission_type[20]->display_type == 1){

				$order_soil_report_bg = '';
				if($consent_info[$i]->order_soil_report == 'N/A') { $order_soil_report_bg='style="background-color:#FFFFFF; color:black"';} 
				if($consent_info[$i]->order_soil_report == 'Received') {$order_soil_report_bg='style="background-color:#90ee90"';}
				if($consent_info[$i]->order_soil_report == 'Sent') {$order_soil_report_bg='style="background-color:#70B5FF"';}
				$message .= '<td '.$order_soil_report_bg.'>'.$consent_info[$i]->order_soil_report.'</td>';

			} if($user_permission_type[21]->display_type == 1){

				$septic_tank_approval_bg = '';
				if($consent_info[$i]->septic_tank_approval == 'REQ'){ $septic_tank_approval_bg = 'style="background-color:red;color:white"';}
				if($consent_info[$i]->septic_tank_approval == 'N/A'){ $septic_tank_approval_bg = 'style="background-color:#90ee90"';} 
				if($consent_info[$i]->septic_tank_approval == 'SENT'){ $septic_tank_approval_bg = 'style="background-color:blue"';}
				if($consent_info[$i]->septic_tank_approval == 'RECEIVED'){ $septic_tank_approval_bg = 'style="background-color:#90ee90"';}

				$message .= '<td '.$septic_tank_approval_bg.'>'.$consent_info[$i]->septic_tank_approval.'</td>';
			} if($user_permission_type[70]->display_type == 1){
				$message .= '<td>'.$consent_info[$i]->drainage_testing.'</td>';
			} if($user_permission_type[22]->display_type == 1){
	
				$dev_approval_bg = '';
				if($consent_info[$i]->dev_approval == 'REQ'){ $dev_approval_bg = 'style="background-color:red;color:white"';} 
				if($consent_info[$i]->dev_approval == 'N/A'){ $dev_approval_bg = 'style="background-color:#90ee90"';} 
				if($consent_info[$i]->dev_approval == 'PRE SENT'){ $dev_approval_bg = 'style="background-color:blue"';}
				if($consent_info[$i]->dev_approval == 'PRE REC'){ $dev_approval_bg = 'style="background-color:yellow; color:black"';}
				if($consent_info[$i]->dev_approval == 'FULL SENT'){ $dev_approval_bg = 'style="background-color:blue"';}
				if($consent_info[$i]->dev_approval == 'FULL REC') { $dev_approval_bg = 'style="background-color:#90ee90"';}
				$message .= '<td '.$dev_approval_bg.'>'.$consent_info[$i]->dev_approval.'</td>';

			} if($user_permission_type[32]->display_type == 1){

				$landscape_bg = '';
				if($consent_info[$i]->landscape == 'REQ') {$landscape_bg = 'style="background-color:red;color:white"';}
				if($consent_info[$i]->landscape == 'N/A') {$landscape_bg = 'style="background-color:#90ee90"';} 
				if($consent_info[$i]->landscape == 'SENT') {$landscape_bg = 'style="background-color:blue"';}
				if($consent_info[$i]->landscape == 'RECEIVED') {$landscape_bg = 'style="background-color:#90ee90"';}
				$message .= '<td '.$landscape_bg.'>'.$consent_info[$i]->landscape.'</td>';

			} if($user_permission_type[33]->display_type == 1){

				$mss_bg = '';
				if($consent_info[$i]->mss == 'REQ') { $mss_bg = 'style="background-color:red; color:white"';} 
				if($consent_info[$i]->mss == 'DONE') { $mss_bg = 'style="background-color:#90ee90"';}
				$message .= '<td '.$mss_bg.'>'.$consent_info[$i]->mss.'</td>';
				...
				if($consent_info[$i]->unconditional_date == '0000-00-00'){
					...
					$message .= '<td style="background-color:#90ee90; color:white;">'.$this->wbs_helper->to_report_date($consent_info[$i]->unconditional_date).'</td>';
				}
			} if($user_permission_type[26]->display_type == 1){
				if($consent_info[$i]->handover_date == '0000-00-00'){
					$message .= '<td>&nbsp;</td>';
				}else{
					$message .= '<td style="background-color:#90ee90; color:white;">'.$this->wbs_helper->to_report_date($consent_info[$i]->handover_date).'</td>';
				}
			} if($user_permission_type[27]->display_type == 1){
				...
				if($consent_info[$i]->settlement_date == '0000-00-00'){
					...
					$message .= '<td style="background-color:#90ee90; color:white;">'.$this->wbs_helper->to_report_date($consent_info[$i]->settlement_date).'</td>';
				}
			} if($user_permission_type[67]->display_type == 1){
				$message .= '<td>'.$consent_info[$i]->for_sale_sign.'</td>';
			} if($user_permission_type[68]->display_type == 1){
